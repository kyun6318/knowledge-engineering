...
> - Cypher 쿼리(`03.graphrag/separate/v7/graphrag/07_neo4j_schema.md` Q2)에서 `stage_label`로 참조한다.
...
1. **Rule 기반**: `HIRING_CONTEXT_TO_SIGNALS` 매핑 테이블 적용 (`03.graphrag/separate/v7/graphrag/07_neo4j_schema.md` Q4 참조)
2. **LLM 기반**: JD 텍스트에서 SituationalSignal 직접 추출 (14개 taxonomy 기반, `01.ontology/v24/02_candidate_context.md` §2.3 정의)
...
> 이 규칙은 `03.graphrag/separate/v7/interface/00_data_contract.md`의 JSON Contract과 일치한다.
...
>
> 이 듀얼 시나리오는 `03.graphrag/separate/v7/graphrag/07_neo4j_schema.md`의 3.2M 기준 추정과 연결된다.