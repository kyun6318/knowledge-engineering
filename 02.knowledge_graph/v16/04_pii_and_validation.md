...
> **[v14 CRITICAL] PII 처리 책임 경계 미확정**: PII 마스킹의 주체가 S&F 팀인지 데이터 플랫폼인지 미확정 상태이다. **Phase 0 시작 전(W0)에 데이터 플랫폼 팀과 PII 처리 책임 경계를 확정**해야 하며, 이 결정이 Phase 1 PII 마스킹 모듈 구현 일정에 직접적 영향을 미친다. Go/No-Go 의사결정 항목으로 추가 필요 (→ `interface/01_go_nogo_decisions.md`).
>
> 가능한 경우 본 문서의 PII 처리 프로세스를 데이터 플랫폼에 이관. 추후 개인 정보 포함 데이터 처리가 필요할 경우를 위해 현재 설계를 유지.
>
...
> v11 리뷰에서 “010-- 하나로는 변형 패턴 미탐지” (Low) 지적.
...
PHONE_PATTERNS = [
    ...
    r'\b(?:\+82[-\s]?)?0?1[016789][-.\s)]*\d{3,4}[-.\s]?\d{4}\b',  # [v15] \b 추가: "1024×768" 등 false positive 방지
...
def validate_masking(text: str) -> bool:
    ...
    patterns = [
        # 전화번호 - v12 종합 패턴
        r'\b(?:\+82[-\s]?)?0?1[016789][-.\s)]*\d{3,4}[-.\s]?\d{4}\b',  # [v15] \b 추가
        r'0[2-6][0-9]?[-.\s]?\d{3,4}[-.\s]?\d{4}',
        # 주민번호
        r'\d{6}-\d{7}',
