# v11 온톨로지 기반 추출 파이프라인 설계 — DB 기반 재설계

> v11 CompanyContext / CandidateContext / Graph Schema에 정합하는 추출 파이프라인.
...
> 개정일: 2026-03-10 (v9 — v11 온톨로지 정합: 3-Tier 비교 전략 통일, normalize_skill() synonyms 추가, 전공 threshold 0.75, confidence 세분화, compare_majors/compute_skill_overlap 하이브리드 추가, 모니터링 지표 반영)
...
| 원칙 | 설명 |
|---|---|
| **v11 스키마 정합** | 추출 결과가 v11 JSON 스키마(CompanyContext, CandidateContext)에 직접 매핑. v11 `00_data_source_mapping.md`가 DB↔온톨로지 필드 매핑의 정본 |
...
| **3-Tier 비교 전략 (Embedding-first Comparison)** | Tier 1: 대학교/회사명/산업 코드 → code-hub CI Lookup. Tier 2: 스킬 → 경량 정규화(CI+synonyms) + 임베딩 fallback. Tier 3: 전공/직무명/롱테일 스킬 → 임베딩 유사도 전용 (정규화 시도하지 않음). v11.1 `00_data_source_mapping.md` §1.5와 정합 **(v9 변경)** |
...
[Pipeline A: CompanyContext 생성 — v9 변경]
    job-hub + code-hub + NICE → CompanyContext JSON
    ├─[1] 구조화 데이터 조회 (DB 직접)
    │   ├─ tech_stack (DB 조회 → Tier 2 경량 정규화: CI+synonyms+임베딩) — LLM 불필요
    │   ├─ industry (overview.industry_codes → code-hub INDUSTRY Tier 1) — LLM 불필요
    │   ├─ career_types, education, designation (requirement, overview) — LLM 불필요
    │   └─ company_profile (NICE Lookup)
    ├─[1.5] 비정형 값 비교 준비 (v9, v11.1 정합)
    │   ├─ skill 원본 값 → normalize_skill() → Tier 2 경량 정규화 (CI + synonyms)
    │   ├─ 정규화 성공 → code-hub skill_id 기반 비교
    │   ├─ 정규화 실패 → 원본 유지, 임베딩 벡터 생성 (§4.3에서 비교)
    │   └─ work_fields (자유 텍스트 직무명) → Tier 3 임베딩 전용
    ├─[2] stage_estimate (Rule + LLM)
    ├─[3] LLM 추출 (vacancy + role_expectations — 정형 필드 사전 제공, 토큰 44% 절감)
    ├─[4] operating_model (키워드 + LLM)
    ├─ domain_positioning (Optional)
    └─ structural_tensions (크롤링 데이터 확보 시 활성화)

    ▼

[Pipeline B: CandidateContext 생성 — v9 변경]
    resume-hub + code-hub + NICE → CandidateContext JSON
    ├─ 중복 감지 (SiteUserMapping 기반)
    ├─ experiences[] 추출
    │   ├─[1] 기본 정보 (DB 직접: Career 엔티티)
    │   ├─[1] tech_stack (DB 조회 → Tier 2 경량 정규화) — LLM 불필요
    │   ├─[1.5] 비정형 값 비교 준비 (v9, v11.1 정합)
    │   │   ├─ Skill 원본 값 → normalize_skill() → Tier 2 (CI + synonyms)
    │   │   ├─ 정규화 실패 → 원본 유지, 임베딩 벡터 생성
    │   │   └─ 전공/직무명 자유 텍스트 → Tier 3 임베딩 전용
    │   ├─[2] scope_type, outcomes (LLM — positionGradeCode 힌트 제공)
    │   ├─[2] situational_signals (LLM + taxonomy)
    │   └─[3] past_company_context (BRN 기반 NICE 직접 매칭 — 매칭률 80-90%)
    ├─ role_evolution (LLM)
    ├─ domain_depth (LLM)
...
| 기술 정규화 | 기술 사전 2,000개 + fuzzy | **3-Tier 비교 전략**: Tier 1(CI Lookup) + Tier 2(경량 정규화+임베딩) + Tier 3(임베딩 전용) | v11.1 `00_data_source_mapping.md` §1.5 정합 **(v9 변경)** |
...
def extract_company_fields_from_db(job_id: str) -> dict:
    ...
    # tech_stack: job-hub.skill → Tier 2 경량 정규화 (v9 변경: v11.1 §1.3 정합)
    tech_stack = []
    for skill in skills:
        normalized = normalize_skill(skill.name)  # Tier 2: CI + synonyms → 미매칭 시 원본 유지
...
# v9 신규: 비교 방법별 세분화된 confidence (v11.1 §6.4 정합)
NORMALIZATION_CONFIDENCE = {
    "code_hub_ci":       0.95,   # Tier 1: code-hub CI 매칭
    "code_hub_synonyms": 0.85,   # Tier 2: code-hub synonyms 매칭
    "embedding_high":    0.80,   # Tier 2/3: embedding similarity >= 0.90
    "embedding_mid":     0.70,   # Tier 2/3: 0.80 <= similarity < 0.90
    "embedding_low":     0.60,   # Tier 3: 0.75 <= similarity < 0.80 (전공 등)
    "unmatched":         0.0,    # threshold 미달: 매칭에서 제외
}
```
...
>
> **(v9 변경)**: `NORMALIZATION_CONFIDENCE` 추가 — v11.1 `00_data_source_mapping.md` §6.4의 비교 방법별 신뢰도 테이블을 반영. 정규화/비교 결과의 confidence를 비교 방법에 따라 세분화.
...
def map_career_to_basic(career, skills) -> dict:
    ...
    # tech_stack: Skill → 3-Tier 비교 전략 (v9 변경: Tier 2 경량 정규화)
    tech_stack = []
    for skill in skills:
        if skill.career_id == career.id:
            normalized = normalize_skill(skill)  # Tier 2: CI + synonyms 정규화 적용
            tech_stack.append(normalized)
...
- **v9 변경**: DB 값이 비표준화된 상태이므로 3-Tier 비교 전략 적용 (v11.1 정합)
- **커버리지**: company 100% (Tier 1 정규화), period 100%, tech_stack 90%+ (Tier 2 경량 정규화+임베딩)
...
### 4.3 Entity Resolution + 3-Tier 비교 전략 **(v9 변경: v11.1 정합)**

> **v9 핵심 변경**: v8.1의 2-Tier 정규화를 v11.1 `00_data_source_mapping.md` §1.5의 **3-Tier 비교 전략**으로 통일한다.
> DB 데이터가 표준화되지 않은 상태임을 전제로, **대상의 특성에 따라 비교 전략을 분리**한다.
> 모든 항목을 코드 정규화하려는 접근은 ROI가 낮으므로, 정규화가 효과적인 대상에만 적용하고 나머지는 임베딩 유사도로 비교한다.

#### 4.3.0 3-Tier 비교 전략 (Embedding-first Comparison) **(v9 변경)**

| Tier | 대상 | 비교 방법 | 근거 | 비용 |
|---|---|---|---|---|
| **Tier 1: 정규화 적합** | 대학교명, 회사명, 산업 코드 | code-hub Lookup (CI 매칭) | 유한 집합, 명확한 정체성, 오매칭 위험 낮음 | **0** (룰 기반) |
| **Tier 2: 경량 정규화 + 임베딩** | 상위 스킬 (Java, Python 등) | code-hub CI 매칭 시도 → 미매칭 시 임베딩 | 상위 50~100개는 CI 매칭 가능, 롱테일은 임베딩 | **embedding API** |
| **Tier 3: 임베딩 전용** | 전공, 직무명(자유 텍스트), 롱테일 스킬 | 임베딩 cosine similarity | 표현 다양성 높음, 정규화 시 거짓 동일성 위험 | **embedding API** |

> **Tier 3이 정규화하지 않는 이유** (v11.1 A9):
> - "컴퓨터공학" vs "컴퓨터과학"은 **다른 전공**인데 코드 정규화하면 동일 처리됨
...
##### Tier 2/3: Embedding 유사도 (Threshold-Based Continuous Score) **(v9 변경)**
...
SIMILARITY_THRESHOLDS = {
    "skill": 0.85,    # Tier 2 스킬: 높은 threshold (동의어가 명확)
    "major": 0.75,    # Tier 3 전공: 낮은 threshold (의미적 유사성이 넓음) **(v9 변경: 0.80→0.75, v11.1 §1.5 정합)**
    "role":  0.80,    # Tier 3 직무: 중간 threshold (직무명 변형 다양)
}
...
def normalize_skill(raw_skill_name: str, site_type: str = "JOBKOREA") -> dict:
    """
    Tier 2 스킬 정규화: CI 매칭 + synonyms 매칭 → 미매칭 시 원본 유지.
    v9 변경: v11.1 §1.3 normalize_skill()과 동일한 시그니처/로직.
    fuzzy 매칭, 한영 사전 등 복잡한 정규화는 ROI가 낮아 적용하지 않는다.
    """
    ...
    # 3. 미매칭 → 원본 유지 (비교는 임베딩으로, §4.3 compute_skill_overlap 참조)
    return {"skill_id": None, "name": raw_skill_name, "normalized": False}


def normalize_role(position_title_code: str, position_title: str) -> dict:
    """Tier 3 직무 정규화: code-hub 코드 참조 → 미매칭 시 임베딩 전용 비교."""
    # Step 1: code-hub JOB_CLASSIFICATION 코드가 있으면 직접 참조 (Tier 1)
    ...
    # Step 2: 자유 텍스트 직무명은 Tier 3 임베딩 전용 비교 (정규화 시도 안 함)
    # v9 변경: 코드 없는 자유 텍스트 직무명은 임베딩 유사도로만 비교
    if position_title:
...
def compare_majors(vacancy_preferred_majors: List[str], candidate_majors: List[str]) -> dict:
    """
    Tier 3 전공 비교: 정규화하지 않고 임베딩 유사도로 비교한다.
    v9 신규: v11.1 §1.5 compare_majors()와 동일.
    ...
    matches = compute_embedding_similarity_batch(
        ...
        candidate_majors,
        threshold=0.75  # Tier 3 전공: 의미적 유사성이 넓으므로 낮은 threshold
    )

    best_score = max([m["similarity"] for m in matches], default=0.0)
    return {"score": best_score, "matches": matches}


def compute_embedding_similarity_batch(
    texts_a: List[str], texts_b: List[str], threshold: float = 0.80
) -> List[Dict]:
    """
    두 텍스트 집합 간 임베딩 유사도를 계산하고 threshold 이상인 쌍을 반환한다.
    v9 신규: v11.1 §1.5 compute_embedding_similarity_batch()와 동일.
...
### 4.4 Skill/Role/Major 정규화 — Graph 적재 시 적용 **(v9 변경: 3-Tier 구조)**

> **v9 변경**: v8.1의 단일 정규화 함수를 **3-Tier 비교 전략에 맞게 분리**하여 Graph 적재 시 각 Tier의 결과를 구분하여 저장한다.

```python
def normalize_and_merge_skill(tx, skill_data: dict):
    """Graph 적재 시 3-Tier 비교 결과에 따라 Skill 노드 MERGE (v9 변경)"""
    if skill_data["match_method"] == "code_hub_ci":
        # Tier 2 CI 매칭 성공: canonical 이름으로 MERGE, confidence 0.95
        tx.run("""
            ...
            SET s.category = COALESCE(s.category, $category),
                ...
                s.match_method = 'code_hub_ci',
                s.normalization_confidence = 0.95,
                s.updated_at = datetime()
        """, canonical_name=skill_data["name"],
            raw_name=skill_data["raw_name"],
            category=skill_data.get("category"))

    elif skill_data["match_method"] == "code_hub_synonyms":
        # Tier 2 synonyms 매칭 성공: canonical 이름으로 MERGE, confidence 0.85
        tx.run("""
            MERGE (s:Skill {name: $canonical_name})
            SET s.category = COALESCE(s.category, $category),
                s.aliases = CASE WHEN $raw_name IN s.aliases THEN s.aliases
                                 ELSE COALESCE(s.aliases, []) + [$raw_name] END,
                s.match_method = 'code_hub_synonyms',
                s.normalization_confidence = 0.85,
                s.updated_at = datetime()
        """, canonical_name=skill_data["name"],
            raw_name=skill_data["raw_name"],
            category=skill_data.get("category"))
        ...
        # Tier 2 미매칭 / Tier 3 임베딩 전용: 원본 이름으로 MERGE, 임베딩으로 비교
        tx.run("""
            MERGE (s:Skill {name: $raw_name})
            SET s.match_method = 'embedding_pending',
                s.normalization_confidence = null,
                s.needs_review = true,
                s.updated_at = datetime()
        """, raw_name=skill_data["raw_name"])


def normalize_and_merge_major(tx, major_data: dict):
    """Graph 적재 시 전공 노드 MERGE — Tier 3 임베딩 전용 (v9 신설)"""
    # 전공은 정규화하지 않고 원본 유지, 비교 시 임베딩 유사도 사용 (v11.1 A9)
    tx.run("""
        MERGE (m:Major {name: $raw_name})
        SET m.match_method = 'embedding_only',
            m.updated_at = datetime()
    """, raw_name=major_data["raw_name"])
```

- **Graph 효과**: "자바", "JAVA", "java" → 단일 `Skill {name: "Java"}` 노드로 MERGE (CI 매칭)
- **aliases 관리**: 원본 값을 aliases 배열에 보존하여 역추적 가능
- **match_method 기록**: 각 노드에 매칭 방법(`code_hub_ci`, `code_hub_synonyms`, `embedding_pending`, `embedding_only`)을 기록하여 비교 전략 추적 가능
- **Tier 3 전공/직무**: 정규화하지 않고 원본 저장, `compare_majors()` / 임베딩 유사도로 비교 (§4.3 참조)
- **normalization_confidence**: NORMALIZATION_CONFIDENCE 딕셔너리(§2.2)에 정의된 값 적용
...
## 5. Pipeline D: MappingFeatures 계산 **(v9 변경: 하이브리드 비교)**
...
### 5.4 compute_skill_overlap() — 하이브리드 비교 **(v9 신설: v11.1 §4.3 정합)**

> v11.1 `00_data_source_mapping.md` §4.3의 `compute_skill_overlap()` 하이브리드 구현.
> 정규화된 스킬은 코드 기반 exact match, 미정규화 스킬은 임베딩 유사도로 비교한다.

```python
def compute_skill_overlap(jd_skills: list[dict], cand_skills: list[dict]) -> dict:
    """
    v9: 하이브리드 스킬 오버랩 계산 (v11.1 §4.3 정합).
    정규화 성공 스킬 → 코드 기반 exact match.
    미정규화 스킬 → 임베딩 cosine similarity.
    """
    # 1. 정규화 성공 스킬 분리
    jd_normalized = [s for s in jd_skills if s.get("match_method") in ("code_hub_ci", "code_hub_synonyms")]
    jd_unmatched = [s for s in jd_skills if s not in jd_normalized]
    cand_normalized = [s for s in cand_skills if s.get("match_method") in ("code_hub_ci", "code_hub_synonyms")]
    cand_unmatched = [s for s in cand_skills if s not in cand_normalized]

    # 2. Tier 1/2 정규화 스킬: code 기반 exact match
    jd_codes = {s["name"] for s in jd_normalized}
    cand_codes = {s["name"] for s in cand_normalized}
    exact_matches = jd_codes & cand_codes
    exact_score = len(exact_matches) / len(jd_codes) if jd_codes else 0.0

    # 3. 미정규화 스킬: 임베딩 cosine similarity (Tier 3)
    embedding_matches = []
    if jd_unmatched and cand_unmatched:
        embedding_matches = compute_embedding_similarity_batch(
            texts_a=[s["raw_name"] for s in jd_unmatched],
            texts_b=[s["raw_name"] for s in cand_unmatched],
            threshold=SIMILARITY_THRESHOLDS["skill"],  # 0.85
        )
    embedding_score = len(embedding_matches) / len(jd_unmatched) if jd_unmatched else 0.0

    # 4. 가중 합산
    total_jd = len(jd_normalized) + len(jd_unmatched)
    if total_jd == 0:
        return {"overlap_ratio": 0.0, "exact_matches": [], "embedding_matches": []}

    weighted_overlap = (
        len(exact_matches) + len(embedding_matches)
    ) / total_jd

    return {
        "overlap_ratio": round(weighted_overlap, 3),
        "exact_matches": list(exact_matches),
        "embedding_matches": embedding_matches,
        "exact_count": len(exact_matches),
        "embedding_count": len(embedding_matches),
        "total_jd_skills": total_jd,
    }
```

- **하이브리드 전략**: 정규화 성공 스킬은 정확한 코드 비교, 실패 스킬은 임베딩으로 보완
- **v7 대비 변경**: v7은 단순 문자열 매칭이었으나, v9는 3-Tier 비교 결과를 활용
- **confidence 적용**: exact match(0.95), embedding match(0.70~0.80) — NORMALIZATION_CONFIDENCE §2.2 참조
...
| Embedding (3-Tier 비교) **(v9)** | ~958K 유니크 (캐시 적용) | $0.0065/1M tok | **~$0.06** | — | 신규 |
...
| tech_stack | **90%** **(v8: code-hub + 3-Tier 비교)** | 10% | Tier 2 경량 정규화(CI+synonyms) + 미매칭 시 임베딩 **(v9)** |
...
**전체 가중 비율**: DB 조회 ~35% + 3-Tier 비교 ~10%, LLM ~55% (v7: Rule 25%, LLM 75%) **(v9 변경)**

### JD(CompanyContext) 추출 기준

| 추출 대상 | DB 조회 | LLM | 비고 |
|---|---|---|---|
| tech_stack | **100%** **(v8: code-hub + 3-Tier 비교)** | 0% | DB 조회 + 3-Tier 비교 전략 **(v9)** |
...
## 10. 비정형 값 비교 품질 모니터링 **(v9 신설: v11.1 §6.4 정합)**

> v11.1 `00_data_source_mapping.md` §6.4에 정의된 모니터링 지표를 파이프라인에 통합한다.

### 10.1 모니터링 지표

| 지표 | 설명 | 목표 | 산출 방법 |
|---|---|---|---|
| **스킬 코드 매칭률** | 전체 스킬 중 Tier 2 CI/synonyms 매칭 성공 비율 | ≥ 70% | `count(match_method in ['code_hub_ci', 'code_hub_synonyms']) / count(all_skills)` |
| **임베딩 매칭 커버리지** | Tier 2 미매칭 + Tier 3 스킬 중 임베딩 threshold 이상 매칭된 비율 | ≥ 85% | `count(embedding_sim >= 0.85) / count(embedding_candidates)` |
| **임베딩 매칭 정확도** | 임베딩 매칭 결과의 수동 샘플링 정확도 (Phase 1-2) | ≥ 90% | 500건 랜덤 샘플링 → 수동 검증 |
| **전공/직무 유사도 분포** | 전공/직무 임베딩 유사도 히스토그램 | threshold 근처 집중 없음 | `histogram(similarities, bins=[0.5, 0.6, 0.7, 0.75, 0.8, 0.9, 1.0])` |

### 10.2 수집 및 알람

```python
def collect_normalization_metrics(batch_results: list[dict]) -> dict:
    """v9: 배치 처리 후 정규화 품질 지표 수집."""
    total_skills = 0
    code_matched = 0
    ...
    for result in batch_results:
        for skill in result.get("skills", []):
            total_skills += 1
            if skill["match_method"] in ("code_hub_ci", "code_hub_synonyms"):
                code_matched += 1
            else:
                embedding_candidates += 1
                if skill.get("embedding_similarity", 0) >= SIMILARITY_THRESHOLDS["skill"]:
                    embedding_matched += 1

    return {
        "skill_code_match_rate": code_matched / total_skills if total_skills else 0,
        "embedding_coverage": embedding_matched / embedding_candidates if embedding_candidates else 0,
...
- **알람 조건**: 스킬 코드 매칭률 < 60% 또는 임베딩 커버리지 < 75% 시 경고
- **Phase 1-2에서 임베딩 정확도 수동 검증**: 500건 샘플링하여 threshold 적정성 검증 → 필요 시 조정
