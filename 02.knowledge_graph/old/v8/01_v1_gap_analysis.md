...
> v1 계획이 v10 온톨로지 요구사항에 맞지 않는 부분을 식별하고, v2/v3~v7에서 수정한 방향을 정리한다.
>
...
| 추출 대상 | v1 접근 | v10 요구 | v8 실제 난이도 |
|---|---|---|---|
| 회사명, 직무명, 기간 | Rule + NER | 동일 | **제거** — resume-hub DB 직접 조회 **(v8 변경)** |
| tech_stack | 기술 사전 + Fuzzy | 동일 + 정규화 | **2-Tier 정규화** — Tier 1(case-insensitive) + Tier 2(embedding 유사도) **(v8.1 변경)** |
...
v8에서의 하이브리드 (DB 기반 재정의):
- **DB 조회**: 회사명, 직무명, 기간, 기술 스택, 학력, industry, career_types → **LLM 불필요**
- **2-Tier 정규화** **(v8.1 신설)**: Tier 1(대학교/회사명/상위 스킬 — case-insensitive) + Tier 2(스킬/전공/직무 — embedding 유사도)
- **Rule**: stage 규칙 판정, NICE 팩트 조회
- **Embedding**: domain_fit (cosine similarity), Chapter/Vacancy vector search, **Tier 2 정규화 유사도**
...
- Tier 1/2 정규화: **10-15%** (비표준 값 정규화) **(v8.1 신설)**
- Rule: **10-15%** (stage 판정 등)
- Embedding: **10-15%** (유사도 기반 피처 + Tier 2 정규화)
...
| 코드 정규화 | 기술 사전 2,000개 + fuzzy matching | **2-Tier 정규화**: Tier 1(대학교/회사명/상위 스킬 — case-insensitive) + Tier 2(스킬/전공/직무 — embedding 유사도) + code-hub 코드 참조 **(v8.1 변경)** |
| NICE 매칭 | 회사명 fuzzy match (60%) | **businessRegistrationNumber 직접 매칭 (80-90%)** |

#### Pipeline A (CompanyContext) 변경

- tech_stack: `job-hub.skill` → 2-Tier 정규화(code-hub HARD_SKILL + embedding 유사도) → **LLM 불필요** **(v8.1 변경)**
...
| 기술 사전 2,000개 구축 | **대폭 축소 → 2-Tier 정규화로 대체** | Tier 1: alias 사전(~50개 상위 카테고리) + Tier 2: code-hub HARD_SKILL 기반 canonical embedding 사전(~2,000개) **(v8.1 변경)** |
...
| **변경** | R2.10 Entity Resolution (Medium→Medium 유지) | DB 값이 비표준이므로 2-Tier 정규화 필요 **(v8.1 변경)** |
