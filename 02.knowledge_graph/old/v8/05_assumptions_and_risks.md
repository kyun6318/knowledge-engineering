...
| **A25** | Tier 2 embedding similarity threshold | **스킬 0.85, 전공 0.80, 직무 0.80** **(v8.1 신규)** | 정규화 정확도 | threshold가 너무 높으면 정규화율 하락 (매핑 안 됨), 너무 낮으면 오매핑 증가 → Phase 0에서 캘리브레이션 |
...
### 2.10 [Medium] Entity Resolution — 비표준 데이터 정규화 **(v8.1 변경: Low→Medium 재조정)**

**리스크**: DB 데이터가 비표준 상태. "자바"/"JAVA"/"java", "서울대"/"서울대학교", "PM"/"프로덕트매니저" 등 동일 엔티티의 변형이 다수 존재

**v8.1 완화**: 2-Tier 정규화 전략 도입
- **Tier 1 (정규화)**: 대학교, 회사명, 상위 스킬 카테고리 → case-insensitive + alias 사전
  - 대학교 alias ~200개, 회사명 alias ~500개, 상위 스킬 ~50개
  - 비용 0, 정확도 높음 (95%+)
- **Tier 2 (Embedding 유사도)**: 스킬, 전공, 직무 → canonical embedding 사전과 cosine similarity
  - threshold 기반 연속 점수: 스킬 0.85, 전공 0.80, 직무 0.80
  - threshold 미달 시 원본 유지 + `needs_review` 플래그
  - 비용 ~$0.06 (전체 대비 무시)

**잔존 리스크**:
- Tier 2 threshold 최적화 필요 (Phase 0 PoC에서 캘리브레이션)
- canonical 사전에 없는 신규 스킬/전공/직무는 정규화 불가
...
### 2.21 [Medium] code-hub 코드 완성도 + 비표준 값 문제 **(v8.1 변경)**

**리스크**: code-hub의 HARD_SKILL 코드가 실제 기술 스택을 충분히 커버하지 못하고, DB 값 자체가 표준화되지 않은 상태 (예: "자바"/"JAVA"/"java" 혼재)

**영향**:
- code null인 경우: code-hub 매핑 불가 → Tier 2 embedding fallback 필요
- 비표준 값: Tier 2 정규화로 canonical 매핑하지만, canonical 사전에 없는 신규 값은 매핑 실패

**완화 전략**:
- Phase 0에서 Skill.code null 비율 + 비표준 값 비율 확인 (A22 검증)
- **Tier 2 embedding 정규화**: code null이거나 비표준인 경우 canonical embedding 사전과 유사도 매핑
- canonical 사전 커버리지 부족 시: 도메인 전문가가 수동으로 canonical 목록 보강
...
| Entity Resolution (BGE-M3 + FAISS) | **방향 전환 → 2-Tier 정규화** **(v8.1 변경)** | Tier 1(alias 사전) + Tier 2(embedding 유사도), BGE-M3 대신 text-multilingual-embedding-002 사용 |
...
| 코드 정규화 | 기술 사전 2,000개 | **2-Tier 정규화**: Tier 1(alias) + Tier 2(embedding) + code-hub 참조 | 비표준 데이터 대응 **(v8.1 변경)** |
...
| 최종 산출물 | Entity + Relation triples | Context JSON + Graph + MappingFeatures | v10 온톨로지의 요구 |
| 타임라인 | 12주 (미명시) | **14~17주 (v8.1, v7의 18~22주에서 단축)** | DB 기반 전처리 제거 + 정규화 모듈 추가 **(v8.1 변경)** |
