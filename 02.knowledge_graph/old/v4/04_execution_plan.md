...
> 개정일: 2026-03-08 (v4 — Phase 0 PoC 범위 확장, Phase 1-4 Graph 적재 보강, 프롬프트 버전 관리, Candidate Shortlisting)
...
**산출물**: 모델 비교 리포트 + 추출 품질 메트릭 + PII 영향 리포트 + Embedding 모델 리포트
...
| PII 전략 | 법무 확인 + 마스킹 영향 테스트 | API (마스킹) / On-premise |
...
Week 11-13: Phase 1-4 (Graph 적재 + Entity Resolution — 2주)
...
> **일정 변경 근거** (v3 대비):
> - Phase 0-2: PII 마스킹 영향 테스트 + embedding 모델 비교 + LLM 호출 전략 비교 추가 (범위 확장, 기간은 기존 1~2주 내 흡수 가능)
> - Phase 1-4: Entity Resolution + 대량 적재 벤치마크 + 적재 전략 분리 = 2주 필요 (v3: 1주 → v4: 2주)
> - Phase 1-5: Candidate Shortlisting 로직 추가 (기존 1주 내 흡수 가능)
> - Phase 2-3 (ML Distillation): 선택적 항목이므로 Phase 2 말미, 품질 평가 결과에 따라 skip 가능
...
| Phase 0 완료 | PII 전략 | 법무 확인 + 마스킹 영향 테스트 | API / On-premise |
