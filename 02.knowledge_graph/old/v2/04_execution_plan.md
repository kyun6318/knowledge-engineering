...
class CompanyContextPipeline:
    def generate(self, job_id: str) -> CompanyContext:
        ...
        # 4. vacancy + role_expectations (LLM)
        vacancy, role_exp = self.llm_extract_vacancy(jd.text)
...
Week 4-5:   Phase 1-1 (전처리 모듈 — 2주)
Week 5-7:   Phase 1-2 (CompanyContext 파이프라인)
Week 7-10:  Phase 1-3 (CandidateContext 파이프라인 — 3주)
Week 10-11: Phase 1-4, 1-5 (Graph + MappingFeatures)

Week 11-13: Phase 2-1 (전체 데이터 처리 — 2~3주)
Week 12-13: Phase 2-2 (품질 평가) — 2-1과 병행
Week 13-14: Phase 2-4 (DS/MLE 서빙 인터페이스)
Week 14-15: Phase 2-3 (ML Distillation — 선택적, 점선)

Week 15+:   Phase 3 (고도화, 지속)
```

**총 MVP 완성**: ~14주 (Phase 0~2, ML Distillation 제외 시 ~13주)
**첫 동작 데모**: ~11주 (Phase 0~1 완료 시점)

> **일정 변경 근거** (v1 대비):
> - Phase 1-1: HWP 파싱 + PII 마스킹 + 기술 사전 구축 = 최소 2주 필요
> - Phase 1-3: 이력서 200건 통합 테스트 + Batch API 연동 = 최소 3주
