...
**영향**: 시나리오 A(API, $497만) ↔ 시나리오 C(On-premise, $1,620만)의 3배 비용 차이
...
### 2.3 [High] PDF/HWP 파싱 품질

**리스크**: 이력서 파싱 품질이 낮으면 Rule 추출 + LLM 입력 품질 모두 저하
...
### 2.7 [Medium] Entity Resolution — 회사명/기술명 정규화
...
### 2.8 [Medium] v4 MappingFeatures 활성화 비율
...
v1 계획의 모든 것이 잘못된 것은 아니다. 아래는 v2에서도 유효하게 활용한다.
...
| 비용 추정 | 1,250만~4,800만 원 | 454만~1,620만 원 | 처리 방식/모델 차이 |
