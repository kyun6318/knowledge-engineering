...
>
> **엔지니어 인건비 미포함**: 위 비용은 직접 비용(API, 인프라, 데이터 레이블링)만 포함. 개발 인력(DE 1명 + MLE 1명, 12주) 인건비는 조직에 따라 다르므로 별도 산정 필요. 참고: 시니어 엔지니어 2명 × 12주 ≈ $30,000~$60,000.
...
|---|---|---|---|---|---|
| **A: Haiku API (권장)** | $605 | $2,160 | $5,840 | **$8,605** | **~1,179만** |
| B: Sonnet API | $2,890 | $2,160 | $5,840 | $10,890 | ~1,492만 |
| C: On-premise | $8,804 | $2,160 | $5,840 | $16,804 | ~2,302만 |
| D: Gemini Flash | $292 | $2,160 | $5,840 | $8,292 | ~1,136만 |

### v1 대비 비용 변화

| 항목 | v1 계획 추정 | v2 계획 추정 | 차이 원인 |
|---|---|---|---|
| LLM 비용 | 500만~3,000만 원 | 83만~396만 원 (API) | v4는 이력서+JD 단위 처리, v1은 150GB 전체 NER/RE |
...
| **총비용** | **1,250만~4,800만 원** | **1,136만~2,302만 원** | 처리 단위/방법의 근본적 차이 |

> **주의**: v1과 v2의 비용 차이는 "v2가 더 효율적"이라는 의미가 아니라,
> **추출 대상과 방법이 근본적으로 다르기 때문**이다.
> v1은 범용 NER/RE로 150GB 전체를 처리하는 비용이고,
> v2는 v4 온톨로지에 맞춘 Context 생성 비용이다.
