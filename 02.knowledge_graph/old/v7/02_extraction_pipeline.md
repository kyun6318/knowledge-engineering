...
### 8.3 LLM 출력 파싱 실패 전략 **(v7 신설)**

> **v7 신설**: 150만 건 이상의 LLM 호출에서 JSON 파싱 실패, 스키마 불일치 등이 발생할 경우의 체계적 대응 전략.
> 기존 8.1의 단순 retry를 확장하여, 파싱 실패에 특화된 3-tier 복구 전략과 모니터링을 정의한다.

#### 파싱 실패율 예상

| 실패 유형 | 예상 비율 | 건수 (150만 호출 기준) | 비고 |
|---|---|---|---|
| JSON 구문 오류 (깨진 JSON) | 2-5% | 3만~7.5만 건 | 중괄호 누락, trailing comma, 문자열 미닫힘 등 |
| 스키마 불일치 (필수 필드 누락) | 1-3% | 1.5만~4.5만 건 | scope_type 누락, outcomes 배열 아닌 문자열 등 |
| 기타 (빈 응답, 프롬프트 거부) | 0.5-2% | 0.75만~3만 건 | content filter, 토큰 초과 등 |
| **합계** | **2-10%** | **3만~15만 건** | |
...
class LLMOutputParser:
    """(v7 신설) LLM 출력 파싱 3-tier 복구 전략"""

    def parse_with_recovery(self, raw_output: str, schema_class, context: dict) -> tuple:
        """
        Returns: (parsed_result, tier_used, partial)
        - tier_used: 1 (정상/json-repair), 2 (temperature 재시도), 3 (skip)
        - partial: True이면 부분 추출 결과
        """
        # ── Tier 1: json-repair 라이브러리로 자동 복구 ──
        try:
            # 먼저 정상 파싱 시도
            parsed = json.loads(raw_output)
        except json.JSONDecodeError:
            try:
                # json-repair로 자동 복구 (trailing comma, 누락 괄호 등)
                repaired = repair_json(raw_output, return_objects=True)
                parsed = repaired
                self.metrics.increment("parse_tier1_repair_success")
            except Exception:
                parsed = None
        ...
        if parsed:
            try:
                ...
                # 필수 필드 존재 시 부분 추출 허용
                partial = self._try_partial_extraction(parsed, schema_class, e)
                if partial:
                    self.metrics.increment("parse_tier1_partial_success")
                    return partial, 1, True
                # Tier 2로 진행
                pass

        # ── Tier 2: temperature 조정 후 재시도 ──
        retry_output = self._retry_with_adjusted_temperature(
            context, temperature=0.0  # temperature를 0으로 낮춰 결정적 출력 유도
        )
        if retry_output:
            try:
                parsed = json.loads(retry_output)
                result = schema_class.model_validate(parsed)
                self.metrics.increment("parse_tier2_success")
                return result, 2, False
            except (json.JSONDecodeError, ValidationError) as e:
                partial = self._try_partial_extraction(
                    json.loads(repair_json(retry_output, return_objects=True))
                    if retry_output else None,
                    schema_class, e
                )
                if partial:
                    self.metrics.increment("parse_tier2_partial_success")
                    return partial, 2, True

        # ── Tier 3: skip + dead-letter ──
        self.metrics.increment("parse_tier3_skip")
        self.dead_letter.handle_failure(
            item_id=context.get("item_id"),
            pipeline=context.get("pipeline"),
            error_type="parse_fail_all_tiers",
            error_msg=f"All 3 tiers failed for {context.get('item_id')}"
        )
        return None, 3, False

    def _try_partial_extraction(self, parsed: dict, schema_class, error: Exception):
        """(v7 신설) Pydantic ValidationError 시 부분 추출 허용.
        필수 필드가 존재하면 optional 필드를 null로 채워 반환."""
        if parsed is None:
            return None

        required_fields = self._get_required_fields(schema_class)
        has_all_required = all(
            parsed.get(f) is not None for f in required_fields
        )

        if has_all_required:
            # optional 필드를 None으로 설정하고 재검증
            for field_name, field_info in schema_class.model_fields.items():
                if field_name not in parsed and not field_info.is_required():
    ...
    def _retry_with_adjusted_temperature(self, context: dict, temperature: float):
        """temperature를 낮춰 LLM 재호출"""
...
#### Pydantic 부분 추출 허용 규칙

| 스키마 | 필수 필드 (반드시 존재) | Optional 필드 (null 허용) |
|---|---|---|
| **Experience 추출** | scope_type, scope_summary | outcomes, situational_signals, failure_recovery |
| **Career Level 추출** | role_evolution.pattern | domain_depth, work_style_signals |
| **Vacancy 추출** | scope_type, role_title | seniority, team_context |

- **원칙**: 필수 필드가 모두 존재하면 optional 필드가 누락되어도 **부분 추출 결과를 수용**한다
- **부분 추출 건은 confidence를 0.3으로 하향** 설정하여 MappingFeatures 계산 시 가중치 감소
- **부분 추출 비율이 전체의 5% 초과 시** 프롬프트 품질 점검 알림 발생

#### 파싱 실패율 모니터링 메트릭 **(v7 신설)**

```python
# (v7 신설) 파싱 실패율 모니터링 — Grafana 대시보드에 추가
PARSE_METRICS = {
    # Tier별 성공/실패 카운터
    "parse_tier1_direct_success":    "Tier 1 정상 파싱 성공",
    "parse_tier1_repair_success":    "Tier 1 json-repair 복구 성공",
    "parse_tier1_partial_success":   "Tier 1 부분 추출 성공",
    "parse_tier2_success":           "Tier 2 temperature 재시도 성공",
    "parse_tier2_partial_success":   "Tier 2 부분 추출 성공",
    "parse_tier3_skip":              "Tier 3 skip (dead-letter)",

    # 집계 메트릭
    "parse_total_calls":             "전체 파싱 시도 횟수",
    "parse_overall_failure_rate":    "전체 파싱 실패율 (Tier 3 / total)",
    "parse_partial_rate":            "부분 추출 비율 (partial / total)",
}

# 알림 규칙
PARSE_ALERT_RULES = [
    {"metric": "parse_overall_failure_rate", "threshold": 0.05, "severity": "WARNING",
     "action": "프롬프트 품질 점검 + LLM 모델 버전 확인"},
    {"metric": "parse_overall_failure_rate", "threshold": 0.10, "severity": "CRITICAL",
     "action": "파이프라인 일시 중단 + 수동 검토"},
    {"metric": "parse_partial_rate", "threshold": 0.05, "severity": "WARNING",
     "action": "프롬프트 출력 형식 지시 강화 검토"},
]
```

- **모니터링 주기**: chunk 단위(1,000건)로 집계, 실시간 Grafana 대시보드 반영
- **Phase 0 PoC 시 기준 설정**: 50건 테스트에서 tier별 비율 측정 → 프로덕션 알림 임계값 확정
- **json-repair 라이브러리**: `pip install json-repair` (MIT 라이선스, LLM 출력 복구에 특화)

### 8.4 Dead-Letter 큐 및 재처리
