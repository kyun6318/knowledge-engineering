...
> **v6 비용 변경**: Embedding 비용이 $6(text-embedding-3-small) → $2(text-multilingual-embedding-002)로 감소. PoC 비용도 $50 → $10으로 감소 (3개 모델 비교 → 1개 모델 검증). 총비용 약 $44 절감.

### 5.1.1 시나리오 A': Haiku 품질 미달 시 Sonnet Batch Fallback **(v7 신설)**

> **v7 신설**: Phase 0 PoC에서 Haiku 추출 품질이 기준 미달일 경우, CompanyContext와 CandidateContext만 Sonnet Batch로 전환하고 나머지는 시나리오 A와 동일하게 유지하는 fallback 시나리오.

| 비용 항목 | 모델/서비스 | 수량 | 단가 | 비용 |
|---|---|---|---|---|
| **CompanyContext LLM** | Sonnet Batch | 10K JD | $0.0015/건 | **$15** |
| **CandidateContext LLM** | Sonnet Batch | 500K 이력서 | $0.00575/건 | **$2,875** |
| **Embedding** | text-multilingual-embedding-002 | 302M 토큰 | $0.0065/1M | **$2** |
| **Embedding 모델 검증 (PoC)** | 확정 모델 검증 | 20쌍 | — | **$10** |
| **Graph DB** | Neo4j AuraDB Professional | 12개월 | $100/월 | **$1,200** |
| **BigQuery** | 서빙 테이블 | 12개월 | $30/월 (추정) | **$360** |
...
> **전환 기준 (v7 신설)**: Phase 0 PoC에서 Haiku 추출 품질이 아래 기준 미달이면 시나리오 A'를 기본 시나리오로 전환한다.
> - scope_type 분류 정확도 < 60%
> - outcomes 추출 F1 < 50%
>
> 시나리오 A' 총비용은 시나리오 B와 동일하나, **의사결정 경로가 다름**: A'는 Haiku를 먼저 시도한 후 품질 미달 시 전환하는 것이고, B는 처음부터 Sonnet을 선택하는 것이다.
...
|---|---|---|---|---|---|
| **A: Haiku API (권장)** | $1,211 | $2,160 | $5,840 | **~$9,211** | **~1,262만** |
| **A': Haiku→Sonnet Fallback (v7 신설)** | $3,522 | $2,160 | $5,840 | **~$11,522** | **~1,579만** |
| B: Sonnet API | $3,340 | $2,160 | $5,840 | $11,522 | ~1,579만 |
| C: On-premise | $9,254 | $2,160 | $5,840 | $17,436 | ~2,389만 |
| D: Gemini Flash | $942 | $2,160 | $5,840 | $8,924 | ~1,223만 |

### v1 대비 비용 변화

| 항목 | v1 계획 추정 | v6 계획 추정 | 차이 원인 |
|---|---|---|---|
| LLM 비용 | 500만~3,000만 원 | 67만~396만 원 (API) | v10은 이력서+JD 단위 처리, v1은 150GB 전체 NER/RE |
...
| **총비용** | **1,250만~4,800만 원** | **1,223만~2,389만 원** | 처리 단위/방법의 근본적 차이 |
