...
| ID | 가정 | 가정값 | 영향 범위 | 잘못되면 |
|---|---|---|---|---|
| **A5** | NICE 매칭률 | 60% | PastCompanyContext 커버리지 | 30%이면 stage_match 피처 대부분 INACTIVE. **(v7 변경)** NICE DB 접근 계약은 Phase 0 이전에 사전 확인 필요 (blocking dependency — 04 문서 Pre-Phase 0 참조) |
...
| **A17** | 이력서 중복률 | 5~10% (v5 추가) | 처리 건수, Graph 품질 | 20%이면 10만 건 중복 처리 낭비, MappingFeatures 왜곡 |
...
| **A16** | Embedding 모델 한국어 분별력 | `text-multilingual-embedding-002` (Vertex AI) 기준 "우수" **(v6 변경)** | 도메인 매칭 정확도 | "양호" 이하이면 Cohere multilingual 또는 BGE-M3 전환, domain_fit 피처 품질 하락. 실측 방법: 20쌍 검증 테스트 (3개 비교 → 확정 모델 검증) |
...
| **A18** | CompanyTalentSignal은 v1에서 의도적으로 제외 **(v6 신설)** | 제외 (v10 A3 참조) | Graph 스키마 범위, MappingFeatures 설계 | 데이터 전제 조건 미충족, 독립성 원칙 강화, 편향 리스크로 v2 이후 도입 검토 |
...
5. **v5 추가**: 법무 결론 미확정 시 **마스킹 기반 API를 기본값으로 Phase 1 진행** (04 문서 §0-3 참조)
...
- **v5 추가**: Phase 0 PoC에서 파싱 → 섹션 분할 → 경력 블록 분리 **단계별 성공률 측정** (04 문서 Phase 0-2)
- **v5 추가**: 경력 블록 분리 정확도 < 50%이면 LLM 기반 섹션 분할 fallback 적용 (비용 $250~500 추가)
...
- **v5 추가**: Deterministic ID + MERGE 패턴으로 재처리 시 별도 DELETE 불필요 (02 문서 §4.6)
- **(v6 변경)**: v10의 새 관계(REQUIRES_ROLE, MAPPED_TO, IN_INDUSTRY)도 MERGE 패턴 적용 대상. 증분 처리 시 이들 엣지가 누락되지 않도록 Cypher MERGE 쿼리에 포함 필수
...
| role_fit | 70-85% | role_evolution 추출 실패. **(v6 변경)** v10의 ScopeType→Seniority 변환 규칙(A1)이 적용되어 매칭 정확도 향상 예상 — scope_type에서 seniority를 규칙 기반으로 도출하므로 role_fit 계산 시 입력 품질이 개선됨 |
...
**리스크**: `text-multilingual-embedding-002` (Vertex AI)가 짧은 한국어 도메인 텍스트("핀테크 백엔드", "이커머스 데이터")에서 충분한 cosine similarity 분별력을 제공하지 못할 수 있음 **(v6 변경)**

**영향**: domain_fit 피처의 정확도 하락 → 매칭 품질 감소

**완화 전략 (v6 변경)**:
- v10에서 `text-multilingual-embedding-002`로 확정되었으므로, Phase 0에서 **확정 모델 검증 + 대안 전환** 접근으로 변경
- 20쌍의 (이력서 도메인, JD 도메인) 텍스트로 cosine similarity 분별력 테스트 (3개 모델 비교 → 확정 모델 단독 검증)
- 분별력 부족 시 → Cohere embed-multilingual-v3.0 또는 BGE-M3로 전환 (비용 증가 허용 범위 내)

### 2.15 [Medium] 이력서 중복이 Graph/서빙 품질에 미치는 영향

> **v5 신설**: v4 리뷰에서 지적된 이력서 중복 처리 리스크.

**리스크**: 동일인의 다중 이력서가 다른 candidate_id로 등록되어 Person 노드가 중복 생성되거나, 동일 candidate_id의 여러 버전이 처리되면 Graph 데이터가 오염됨

**영향**: MappingFeatures에서 동일인이 여러 번 매핑되어 서빙 결과에서 중복 추천 발생

**완화 전략**:
- Phase 0에서 중복률 실측 (동일 candidate_id 다중 버전 + SimHash 유사 이력서)
- 동일 candidate_id: 최신 버전만 처리 (02 문서 §3.4)
- 다른 candidate_id, SimHash > 0.9: 수동 검토 큐로 이동
- Person 노드는 candidate_id 기반 MERGE로 중복 방지 (이미 구현)

### 2.16 [Medium] v10 스키마 진화에 따른 계획 정합성 리스크

> **(v6 신설)**: v5↔v10 교차 검증에서 도출된 스키마 진화 리스크.

**리스크**: 온톨로지 스키마(v10)가 계속 진화하면 파이프라인 코드, Graph 스키마, Cypher 쿼리가 뒤처질 수 있음. 현재도 v10에서 Industry 노드, REQUIRES_ROLE / MAPPED_TO / IN_INDUSTRY 엣지, evidence_chunk 필드 등이 추가되었으며, 향후 추가 변경이 예상됨

**영향**:
- 새로운 노드(Industry), 엣지(REQUIRES_ROLE, MAPPED_TO, IN_INDUSTRY), 필드(evidence_chunk) 등이 추가될 때마다 파이프라인 코드 수정 필요
- Graph 스키마와 온톨로지 스키마 간 불일치 시 데이터 누락 또는 쿼리 오류 발생
- 교차 검증 없이 한쪽만 업데이트하면 정합성 깨짐

**완화 전략**:
- v10 스키마를 **Pydantic 모델로 정의**하여 스키마 변경 시 자동 검증 — 노드/엣지 타입 불일치를 빌드 타임에 감지
- **교차 검증 리뷰를 버전 업 시마다 수행** — 온톨로지(01) 변경 시 파이프라인 계획(02) 영향도 분석 필수
- Graph 스키마 마이그레이션 스크립트를 온톨로지 변경과 함께 작성하여 동기화 보장
- CI/CD에 스키마 호환성 체크 추가: Pydantic 모델 기반 테스트가 새 필드/관계 누락을 자동 탐지

### 2.17 [High] LLM 출력 파싱 실패 **(v7 신설)**

**리스크**: 150만 건 이상의 LLM 호출(CandidateContext 500K × 경력 3건 + CompanyContext 10K)에서 JSON 파싱 실패, 스키마 불일치 등이 2-10% 비율로 발생하여 3만~15만 건의 추가 처리가 필요

**영향**:
- 파싱 실패 건이 dead-letter로 누적되어 수동 검토 부담 증가
- 대량 실패 시 전체 파이프라인 처리 일정 지연 (예상 +1~3일)
- 부분 추출 비율이 높으면 MappingFeatures 품질 하락 (confidence 0.3 건 누적)

**예상 파싱 실패율**:

| 실패 유형 | 비율 | 건수 |
|---|---|---|
| JSON 구문 오류 | 2-5% | 3만~7.5만 |
| 스키마 불일치 | 1-3% | 1.5만~4.5만 |
| 기타 (빈 응답 등) | 0.5-2% | 0.75만~3만 |

**완화 전략**: 02 문서 §8.3 "LLM 출력 파싱 실패 전략" 참조
- 3-tier retry: json-repair → temperature 조정 재시도 → skip + dead-letter
- Pydantic 부분 추출 허용 (필수 필드 존재 시)
- 파싱 실패율 모니터링 메트릭 + Grafana 알림 (5% WARNING, 10% CRITICAL)
- Phase 0 PoC에서 50건 테스트 시 tier별 비율 사전 측정
...
| Embedding 모델 | text-embedding-3-small | text-multilingual-embedding-002 (v6) | v10 확정 모델 반영 |
...
| 스키마 정합성 | 없음 | Pydantic 모델 + 교차 검증 (v6) | v10 스키마 진화 대응 |
| CompanyTalentSignal | 포함 가정 | v1 제외, v2 이후 검토 (v6) | 데이터 전제 조건 미충족 |
| 타임라인 | 12주 (미명시) | 18~22주 (DE+MLE 2명 기준, v7 현실화) | 현실적 일정 반영 + Phase 1 버퍼 |
