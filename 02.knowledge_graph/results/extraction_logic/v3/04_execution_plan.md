...
> 개정일: 2026-03-08 (v3 — 인력 배치, 타임라인 현실화, 운영 전략, 테스트/모니터링 추가)
...
Phase 1: MVP 파이프라인 (7~9주)
    ├─ 전처리 모듈 (2주)
    ├─ CompanyContext 파이프라인 (1~2주)
    ├─ CandidateContext 파이프라인 (3주)
    ├─ Graph 적재 + Vector Index (1주)
    └─ MappingFeatures 계산 (1주)
...
**산출물**: 데이터 프로파일 리포트 — 가정 A1~A5, A11~A12 검증
...
### 1-4. Graph 적재 파이프라인 (1주)

- [ ] CompanyContext → Neo4j 노드/엣지 로더
- [ ] CandidateContext → Neo4j 노드/엣지 로더
- [ ] Entity Resolution (Skill, Role, Organization 정규화)
...
> **v3 변경**: v2의 12~15주 → 14~18주로 현실화. Phase 1 전처리 2주, CandidateContext 3주 반영.

```
Week 1-2:   Phase 0-1, 0-2 (데이터 탐색 + 인프라)
Week 2-3:   Phase 0-2, 0-3 (LLM PoC + 인프라 셋업)
Week 3-4:   Phase 0-4 의사결정 + Phase 1-1 시작 (전처리)

Week 4-6:   Phase 1-1 (전처리 모듈 — 2주)
Week 6-8:   Phase 1-2 (CompanyContext 파이프라인)
Week 8-11:  Phase 1-3 (CandidateContext 파이프라인 — 3주)
Week 11-12: Phase 1-4, 1-5 (Graph + MappingFeatures)

Week 12-15: Phase 2-1 (전체 데이터 처리 — 2~3주)
Week 13-14: Phase 2-2 (품질 평가) — 2-1과 병행
Week 14-15: Phase 2-4 (DS/MLE 서빙 인터페이스)
Week 15-17: Phase 2-3 (ML Distillation — 선택적, 점선)

Week 17+:   Phase 3 (고도화, 지속)
```

**총 MVP 완성**: ~15~17주 (Phase 0~2, ML Distillation 제외 시 ~14~15주)
**첫 동작 데모**: ~12주 (Phase 0~1 완료 시점)

> **일정 변경 근거** (v2 대비):
> - Phase 1-1: HWP 파싱 + PII 마스킹 + 기술 사전 구축 = 최소 2주 필요 (v2: 1주 → v3: 2주)
> - Phase 1-3: 이력서 200건 통합 테스트 + Batch API 연동 = 최소 3주 (v2: 2~3주 → v3: 3주 확정)
> - Phase 2-3 (ML Distillation): 선택적 항목이므로 Phase 2 말미로 이동, 품질 평가 결과에 따라 skip 가능
> - Phase 2-4 (서빙 인터페이스): Phase 2 데이터 처리와 품질 평가 완료 후 진행이 자연스러움
...
| Phase 0 완료 | PII 전략 | 법무 확인 | API / On-premise |
