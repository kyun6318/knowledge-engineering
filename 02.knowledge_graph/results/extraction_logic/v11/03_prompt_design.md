...
>
> v11 신규 문서: v10 리뷰에서 "추출 로직의 핵심인 프롬프트가 전무" (Critical) 지적 해소
...
class CompanyContextExtraction(BaseModel):
    ...
    structural_tensions: Optional[list[StructuralTension]] = Field(
        None, description="감지된 구조적 긴장 (v1에서 대부분 null)"
    )
...
4. **LLM 진정성 체크**: 광고성 문구 vs 실제 운영 방식 구분
   - "애자일 팀"이 구체적 스프린트 언급 없이 단독 사용 → 광고성 → speed null
...
```json
{
  ...
  "structural_tension_evidence": null
}
```
...
```json
{
  ...
  "structural_tensions": ["speed_vs_reliability"],
  "structural_tension_evidence": "빠른 기능 출시와 시스템 안정성 사이의 균형"
}
```
...
```
아래는 후보자의 경력 정보입니다. Career별로 분석해주세요.
...
class CandidateContextExtraction(BaseModel):
    ...
    work_style_signals: Optional[list[str]] = Field(None, description="업무 스타일 키워드 (v1 INACTIVE)")
```
...
**v19 A1 매핑** (scope_type → Seniority 변환):
- IC → JUNIOR/MID (경력 연차 기반)
- LEAD → SENIOR/LEAD
- HEAD → HEAD
- FOUNDER → C_LEVEL (EXECUTIVE 대응)
...
| 파라미터 | CompanyContext | CandidateContext | 비고 |
|---------|---------------|------------------|------|
| model | claude-haiku-4-5 | claude-haiku-4-5 | Phase 0 검증 후 확정 |
| temperature | 0.3 | 0.3 | 재시도 시 0.5 |
| max_tokens | 1,024 | 2,048 | Career 수에 따라 조정 |
| response_format | json | json | Pydantic 검증 |
| batch_mode | true | true | 50% 비용 절감 |
