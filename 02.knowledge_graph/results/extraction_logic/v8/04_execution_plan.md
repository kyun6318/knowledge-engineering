# 실행 계획 v8 — v10 온톨로지 기반 (DB 기반 재설계)
...
Phase 1: MVP 파이프라인 (6~8주) — v8 변경: 8~10주에서 단축
    ├─ DB 커넥터 + 데이터 매핑 + 2-Tier 정규화 (1~2주) — v8.1 변경: 정규화 모듈 추가
...
#### 비표준 값 프로파일링 **(v8.1 신설)**
- [ ] 스킬 비표준 비율 측정: Skill.name에서 동일 스킬의 변형 수 (예: "자바"/"JAVA"/"java" 등)
...
- [ ] Tier 2 embedding threshold 검증을 위한 50건 샘플 수동 매핑 (gold set)
  - 스킬 20건, 전공 15건, 직무 15건
...
### 1-1. DB 커넥터 + 데이터 매핑 + 정규화 (1~2주) **(v8.1 변경: 1주 → 1~2주, 2-Tier 정규화 모듈 추가)**

> v7의 "PDF/DOCX/HWP 파서, 섹션 분할기, 경력 블록 분리기, 기술 사전, 회사 사전" 전체가
> **DB 커넥터 + 데이터 매핑 + 2-Tier 정규화 모듈**로 대체된다.
> **(v8.1 변경)**: DB 값이 비표준 상태이므로 정규화 레이어 추가. Tier 2 canonical embedding 사전 구축 포함.
...
#### 2-Tier 정규화 모듈 **(v8.1 신설)**
- [ ] **Tier 1 정규화 엔진**: case-insensitive 매칭 + alias 사전
  - 대학교 alias 사전 (~200개): "서울대"→"서울대학교", "KAIST"→"한국과학기술원" 등
  - 회사명 alias 사전 (~500개): BRN null fallback용
  - 상위 스킬 카테고리 alias (~50개): "frontend"→"프론트엔드" 등
- [ ] **Tier 2 Canonical Embedding 사전 구축** (1회성)
  - code-hub HARD_SKILL → 스킬 canonical embedding (~2,000개)
  - 전공 마스터 데이터 → 전공 canonical embedding (~500개)
  - code-hub JOB_CLASSIFICATION → 직무 canonical embedding (~300개)
  - Vertex AI `text-multilingual-embedding-002` 배치 임베딩
- [ ] **Tier 2 정규화 엔진**: embedding cosine similarity + threshold 판정
  - similarity threshold: 스킬 0.85, 전공 0.80, 직무 0.80 (Phase 0 PoC에서 캘리브레이션)
  - 캐시 레이어: 동일 텍스트 embedding 결과 캐시 (메모리 → 디스크)
- [ ] **정규화 통합 테스트**: 100건 샘플로 Tier 1/2 정규화 정확도 측정
...
- [ ] **v8.1 변경**: Skill, Role, Major 정규화 — 2-Tier 전략 적용
  - Tier 1: 상위 카테고리 alias 사전 기반 case-insensitive 매칭
  - Tier 2: canonical embedding 유사도 매칭 (Phase 1-1에서 구축한 사전 활용)
  - Graph MERGE 시 정규화된 canonical name으로 통합 (aliases 보존)
  - threshold 미달 노드에 `needs_review = true` 플래그 부여
  - ~~Skill 정규화 사전 2,000개~~ → Tier 2 canonical embedding 사전 (~2,800개)
...
| 테스트 레벨 | 대상 | 기준 | 시점 |
|---|---|---|---|
| **Unit** | DB 커넥터, 코드 매핑, 2-Tier 정규화 모듈 **(v8.1 변경)** | 필드 매핑 정확도 100%, Tier 1 정확도 95%+, Tier 2 precision 90%+ | Phase 1-1 |
...
Week 3-5:   Phase 1-1 (DB 커넥터 + 데이터 매핑 + 2-Tier 정규화 — 1~2주) **(v8.1 변경)**
...
**총 MVP 완성**: ~**14~17주** (Phase 0~2, ML Distillation 제외 시 ~13~15주) **(v8.1 변경: 정규화 모듈 +1주)**
**첫 동작 데모**: ~**12주** (Phase 0~1 완료 시점) **(v8.1 변경)**
...
| Phase 1-1 | 전처리 2주 | DB 커넥터 + 정규화 1~2주 | 0~1주 | 파서/분할기 불필요, 단 2-Tier 정규화 모듈 추가 **(v8.1 변경)** |
...
| **총 MVP** | **18~22주** | **14~17주** | **4~5주** | v8.1: 정규화 모듈 추가로 +1주 **(v8.1 변경)** |
| **첫 동작 데모** | 16주 | **12주** | **4주** | **(v8.1 변경)** |
