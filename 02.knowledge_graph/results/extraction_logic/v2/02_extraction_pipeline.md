...
### 2.3 vacancy — LLM 추출 (필수)

JD에서 scope_type, seniority, team_context 추출은 반드시 LLM이 필요하다.
...
- **비용**: JD 1건당 ~1,500-2,500 토큰 (입력 + 출력)
- **모델**: Claude Haiku 4.5 / Gemini Flash 2.0 (비용 효율)
- **Rule 불가 이유**: scope_type 판별은 문맥 해석이 필수

### 2.4 role_expectations — LLM 추출

```python
ROLE_EXPECTATIONS_PROMPT = """
아래 JD에서 다음을 추출하세요:
...
- **비용**: JD 1건당 ~1,500-2,000 토큰
- **최적화**: vacancy 추출과 **단일 프롬프트로 통합** 가능 → 토큰 절감
- **tech_stack**: LLM 추출 후 기술 사전으로 정규화 (post-processing)

### 2.5 operating_model — 키워드 + LLM 보정
...
```python
# Chapter embedding (이력서 원문 발췌)
for chapter in chapters:
    embedding = embed_model.encode(chapter.evidence_chunk)
    tx.run("""
        MATCH (ch:Chapter {chapter_id: $id})
        SET ch.evidence_chunk_embedding = $emb
    """, id=chapter.chapter_id, emb=embedding)

# Vacancy embedding (JD 원문 발췌)
vacancy_emb = embed_model.encode(vacancy.evidence_chunk)
```

- **Embedding 모델**: text-embedding-3-small ($0.02/1M tokens) 또는 Cohere embed-multilingual-v3.0
...
| vacancy | 0% | **100%** | 문맥 해석 필수 |
| role_expectations | 10% (tech_stack 사전) | **90%** | 대부분 LLM |
