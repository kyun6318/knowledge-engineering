...
>
> v11 대비 주요 변경:
> - M1: Career별 vs 전체 이력 LLM 호출 전략 확정 (→ 01_extraction_pipeline.md §3.4에 기술, 프롬프트 분기는 본 문서 §2.2)
> - S5: structural_tensions, work_style_signals 프롬프트에서 제외
> - C3: operating_model "LLM 진정성 체크" → 단순 confidence 규칙으로 대체
...
# v12 S5: StructuralTension enum 및 관련 필드를 프롬프트에서 제외.
# v1에서 대부분 null이므로 토큰 절약 + 노이즈 방지.
...
class CompanyContextExtraction(BaseModel):
    ...
    # v12 S5: structural_tensions 제거 (v1 INACTIVE)
    # Phase 5 복원 시:
...
### 2.2 User Prompt Template (v12 M1 — 호출 전략별 분기)

> v11 리뷰에서 "Career별 추출 vs 전체 이력 추출의 호출 구조가 미확정" (High) 지적.
> v12에서 Career 수 기반 적응형 호출 전략 확정. 상세 호출 구조는 01_extraction_pipeline.md §3.4 참조.
