...
6. **[v3 신규] SIE 모델 사전 추출**: LLM 프롬프트 투입 전 GLiNER2/NuExtract 1.5로 경력-프로젝트-성과 구조를 사전 추출한다. SIE 추출 결과를 LLM 프롬프트의 컨텍스트로 제공하면 추출 정확도 향상 및 토큰 절감이 기대된다. SIE 미적용 시 기존 프롬프트 그대로 사용. 상세: `00.datamodel/summary/v3.md §3`
...
- For seniority, use exactly one of: JUNIOR, MID, SENIOR, LEAD, HEAD, UNKNOWN
  - JUNIOR: 0~3년 경력, 입문 수준
  - MID: 3~6년 경력, 독립적 업무 수행 가능
  - SENIOR: 7년+ 경력, 기술적 리더십
  - LEAD: 팀 리드, 기술 리딩
  - HEAD: 부서/조직 책임자
  - UNKNOWN: 시니어리티 판별 불가 시. confidence < 0.3과 함께 사용
```
...
class CompanyContextExtraction(BaseModel):
    ...
    # [v17] Seniority enum을 정본(01.ontology)과 통일 (C1)
    # C_LEVEL → HEAD로 변경, MID 추가
    # 정본 정의: 01.ontology/v25/01_company_context.md L125
    seniority: Optional[Literal["JUNIOR", "MID", "SENIOR", "LEAD", "HEAD", "UNKNOWN"]] = Field(
        None, description="채용 포지션의 시니어리티 수준. designation 코드 또는 JD 본문에서 추출. "
                          "MID는 경력 3~6년의 개인 기여자(IC)에 해당")
