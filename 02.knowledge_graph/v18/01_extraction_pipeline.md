...
### [v3 신규] Pipeline E: 외부 데이터 통합 (Phase 5)

LinkedIn/BrightData 외부 데이터를 기존 파이프라인 결과와 교차 매핑:

| 단계 | 입력 | 출력 | 비고 |
| --- | --- | --- | --- |
| E-1 | brightdata_dump_raw_stage0b (2.0M) | LinkedIn 프로필 정규화 | country_code 전부 KR |
| E-2 | linkedin_experience_standardized (2.7M) | Chapter 보강 데이터 | AI 표준화 완료 (confidence/reasoning) |
| E-3 | LinkedIn company_id | Organization 교차 매핑 | 회사명 정규화 3차 패스 |

> Phase 5는 Phase 1-4 완료 후 진행. LinkedIn ↔ resume-hub 동일 인물 매칭 정확도 검증 필요.
...
> - Cypher 쿼리(`03.graphrag/separate/v8/graphrag/07_neo4j_schema.md` Q2)에서 `stage_label`로 참조한다.
...
1. **Rule 기반**: `HIRING_CONTEXT_TO_SIGNALS` 매핑 테이블 적용 (`03.graphrag/separate/v8/graphrag/07_neo4j_schema.md` Q4 참조)
2. **LLM 기반**: JD 텍스트에서 SituationalSignal 직접 추출 (14개 taxonomy 기반, `01.ontology/v25/02_candidate_context.md` §2.3 정의)
...
> 이 규칙은 `03.graphrag/separate/v8/interface/00_data_contract.md`의 JSON Contract과 일치한다.
...
> **[v14→v18 유지] CareerDescription 귀속 정확도 Phase 0 검증 요건**:
...
Pipeline B/B'에서 LLM 추출 전 SIE(GLiNER2/NuExtract 1.5) 모델을 사전 추출 단계로 활용:

| 단계 | 역할 | 비고 |
| --- | --- | --- |
| SIE 사전 추출 | workDetails/careerDescription에서 경력-프로젝트-성과 구조 추출 | GLiNER2 Span 기반, Hallucination 없음 |
| LLM 보완 추출 | SIE 추출 결과를 컨텍스트로 제공하여 scope_type, situational_signals 등 추론 | SIE 결과가 있으면 LLM 부담 감소 |
...
| HWP | hwp5 / pyhwp → Gemini Multimodal 폴백 → **[v14→v18 유지] LibreOffice CLI 변환(hwp→pdf) 경유 3차 폴백** | Cloud Run Job: kg-parse |
...
> **[v14→v18 유지] HWP 파서 Phase 2-0 PoC 추가 요건**: HWP 10건을 Gemini Multimodal로 처리한 결과를 포함. HWP는 한국 고유 포맷으로 Gemini의 지원 수준이 불확실하므로, 실패 시 `hwp5 → LibreOffice CLI(soffice --headless --convert-to pdf) → PDF 파서` 경유 전략을 대안으로 준비.
...
| Chapter | chapter_id, period, scope_type, domain_depth | B/B’ _[v18] (scope_type → seniority 변환은 매칭 시점, 01.ontology §F5)_ |
...
>
> 이 듀얼 시나리오는 `03.graphrag/separate/v8/graphrag/07_neo4j_schema.md`의 3.2M 기준 추정과 연결된다.