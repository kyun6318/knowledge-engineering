...
// Step 2: Chapter -> 공유 노드(SituationalSignal) 관계만 제거 [v15]
MATCH (c:Chapter {chapter_id: $chapter_id})-[r]->(sig:SituationalSignal)
DELETE r

// Step 3: Chapter -> 비공유 노드(Outcome) 관계 + 노드 삭제 [v15]
MATCH (c:Chapter {chapter_id: $chapter_id})-[r]->(owned:Outcome)
DELETE r, owned

// Step 4: Person -> Chapter 관계 + Chapter 삭제 [v15 번호 변경]
MATCH (p:Person)-[r:HAS_CHAPTER]->(c:Chapter {chapter_id: $chapter_id})
DELETE r, c

// Step 5: NEXT_CHAPTER 관계 재연결 [v15 번호 변경]
...
| SituationalSignal | **[v15] 공유** (14개 고정 taxonomy 노드, `01.ontology/v25/02_candidate_context.md` §2.3 (14개 고정 taxonomy 정의), `04_graph_schema.md` §1.7 (노드 구조)) | 관계만 제거, 노드 유지 |
...
> **[v14→v18 유지] Gold Set과 통계 샘플링의 역할 구분**:
> - **Gold Set** (50→200건): 회귀 테스트용. 프롬프트/모델 변경 시 품질 저하를 탐지하는 고정 벤치마크. 도메인 전문가가 수동 라벨링하며, 라벨은 변경하지 않는다.
> - **통계 샘플링** (384건, Phase 2): 품질 검증용. 전체 모집단의 추출 품질을 95% CI ±5%로 추정하기 위한 무작위 샘플. Gold Set과 겹치지 않도록 별도 추출한다.
> - 200건 Gold Set은 회귀 테스트(변화 탐지)에는 충분하나, 전체 모집단의 품질을 통계적으로 대표하지는 않는다. 따라서 두 가지를 병행 운영한다.
...
## 5. LinkedIn 데이터 동기화 전략 (v3 신규)

| 항목 | 전략 | 비고 |
| --- | --- | --- |
| 초기 적재 | brightdata_dump_raw_stage0b 2.0M 전량 처리 | Phase 5 |
| AI 표준화 | linkedin_experience_standardized 2.7M 활용 | 이미 AI 표준화 완료 (confidence/reasoning 포함) |
| 증분 처리 | BrightData 정기 크롤링 시 delta 처리 | 빈도 미정 |
| 교차 매핑 | LinkedIn company_id ↔ resume-hub companyName 매핑 | Organization ER 3차 패스 |

> LinkedIn 데이터는 Phase 5에서 처리. Phase 1-4의 resume-hub/job-hub 파이프라인과 독립적으로 진행 가능.