> 작성일: 2026-03-12
> 01.ontology/results/schema/v25/00_data_source_mapping.md §6에서 이동.
...
> **[v18] Confidence Penalty 규약** (`01.ontology/v25/03_mapping_features.md` 연계):
> | fallback 유형 | confidence penalty | 예시 |
> |---|---|---|
> | 1차 소스 → 2차 소스 | -0.10 | CareerDescription → SelfIntroduction |
> | DB 필드 → LLM 추출 | -0.05 | designation_codes → JD text LLM |
> | 정규화 → 임베딩 매칭 | -0.05 ~ -0.15 | code-hub CI → embedding (유사도별) |
> | 데이터 부재 → 기본값 | -0.20 | null → UNKNOWN |
...
| --- | --- |
| F1 stage_match (→ `01.ontology/v25/03_mapping_features.md` F1 참조) | 회사명->Organization 정규화 (4.48M 고유값), NICE 매핑 |
...
## 7. LinkedIn 외부 데이터 품질 (v3 신규)

| 항목 | 값 | 비고 |
|------|-----|------|
| 총 프로필 | 2,019,293 | country_code 전부 KR |
| 경력 보유 | 68.7% (1,387,533) | 31.3% 빈배열 |
| education 보유 | 41.0% (827,641) | 59% 빈배열 |
| company 유니크 | 933,923개 | 한/영 혼재 심각 |
| title 유니크 | 530,824개 | 직급/직무 혼재 |
| description 채움률 | 31.5% | 텍스트 기반 분석 제한적 |
| city 실질 유효 | ~35% | 64.8% 국가명만 |
| AI 표준화 경력 | 2,695,840건 | confidence/reasoning 포함 |

> LinkedIn 데이터는 resume-hub 대비 채움률이 낮으나, AI 표준화된 경력(linkedin_experience_standardized)을 통해 Organization/Role 교차 검증에 활용 가능.
