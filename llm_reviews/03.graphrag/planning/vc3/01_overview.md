...
> **핵심 원칙**: 하드필터·PII·파싱·임베딩은 S&F 팀이 전담하고, Chapter 기반 그래프 관계·매칭은 GraphRAG 팀이 전담한다.

---

## 1. 분리 배경

v5 실행계획(27주)은 DE 1명 + MLE 1명이 전 파이프라인을 일괄 수행한다. 이 구조의 문제:

1. Phase 2(9주)의 대부분은 파일 파서·LLM Batch 600K·임베딩 등 **그래프와 무관한 ETL 작업**
2. Phase 3에서도 JD 파싱·CompanyContext LLM 추출이 선행되어야 매칭 로직 착수 가능
3. GraphRAG 핵심 역량(`[NEXT_CHAPTER]` 패턴 분석, 5-피처 매칭)이 ETL에 묻혀 희석

**Polyglot Persistence 아키텍처**: S&F는 RDB/Vector DB에, GraphRAG는 Neo4j에 최적화된 저장소를 독립 운영하여, 각 엔진의 주특기를 최대로 활용한다. 두 시스템은 **Data Contract(JSON)와 Event(PubSub)**로만 결합하며, GraphRAG는 "텍스트가 어떻게 파싱/벡터화되었는지"를 전혀 알 필요가 없다.
...
| **하드 필터 API** | 스킬/연차/학력/시니어리티 등 속성 기반 1차 필터 + 벡터 검색 → **ID Top 500~1,000건 반환** |
...
> 73개 태스크 상세 테이블은 `v1/c_01_task_classification.md` 참조

| 팀 | Phase 0 | Phase 1 | Phase 2 | Phase 3 | Phase 4 | **합계** |
|----|---------|---------|---------|---------|---------|--------|
| **S&F** | 10 | 8 | 10 | 6 | 1 | **35** |
| **GraphRAG** | 1 | 7 | 4 | 7 | 10 | **29** |
| **공동** | 2 | 1 | 2 | 3 | 1 | **9** |
