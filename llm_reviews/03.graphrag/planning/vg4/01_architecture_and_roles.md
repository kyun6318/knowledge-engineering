...
1. Phase 2(9주)의 대부분은 파일 파서·LLM 600K·임베디 등 **그래프와 무관한 ETL**
...
> **SLA 책임: IN-list 그래프 매칭 p95 < 2s**

| 범주 | 담당 업무 |
|------|---------|
| **그래프 모델링** | Neo4j 스키마(v19), 인덱스 설계, UNWIND 배치 적재 |
| **그래프 적재** | Person/Chapter/Skill/Role/Organization/Industry, **NEXT_CHAPTER 연결** |
| **Neo4j 인프라** | AuraDB Free→Professional 전환, 사이징(N8), 백업 |
| **서빙 API** | REST API (검색 5종 + 매칭 + 기업 조회), PII 필터 미들웨어 |
| **매칭 알고리즘** | MappingFeatures 5-피처 스코어링, MAPPED_TO 관계, 가중치 튜닝 |
