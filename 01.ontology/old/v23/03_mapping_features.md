...
| SelfIntroduction only | 47.2% | 0.5x | 0.30~0.43 (Low) |
| 둘 다 미보유 | 35.9% | — | INACTIVE |

**영향**: F2가 ACTIVE인 경우에도 47.2%가 SelfIntroduction 경유로 인해 confidence가 0.30~0.43 수준이다. 이는 overall_match_score에서 F2의 실질 기여도를 낮추어, vacancy_fit 가중치 0.30이 명목상보다 실효성이 떨어질 수 있다.
...
| F5 role_fit | `min(seniority_source_confidence, role_evolution_confidence)` — 입력 소스 최소값 |
...
>
> **[v22] 활성 피처 부족 시 low_confidence_flag 정책**: 가중치 재분배 후 활성(ACTIVE) 피처가 2개 이하인 경우, `overall_match_score`를 산출하되 `low_confidence_flag: true`를 부여한다. 서빙 시 low_confidence_flag가 true인 매핑은 별도 처리(예: 결과 목록 하단 배치, UI에서 신뢰도 경고 표시)를 적용한다. 이는 REPLACE 공고에서 vacancy_fit + culture_fit 모두 INACTIVE이고 stage_match도 UNKNOWN인 경우(활성 피처: domain_fit + role_fit 2개)에 해당할 수 있다.

> **[v22] REPLACE 비율 Phase 0 실측 격상**: REPLACE 비율 실측을 Phase 0 **최우선 과제**로 격상한다. REPLACE가 한국 채용 시장에서 흔한 패턴(충원/결원/대체)이므로, 비율이 50%+일 경우 v1 매칭 품질의 절반이 vacancy_fit 없이 결정된다. Phase 0에서 JD 100건 이상의 hiring_context 분포를 실측하여, REPLACE 비율이 30%+ 시 Go/No-Go 의사결정 항목에 포함한다.

### 4.2 freshness_weight 적용 규칙

> §4.2 freshness_weight/ranking_score 서빙 구현 → 03.graphrag/results/implement_planning/separate/v6/graphrag/08_serving.md로 이동

---

## 5. DS/MLE 소비 인터페이스

> §5 BigQuery 서빙 스키마/SQL → 03.graphrag/results/implement_planning/separate/v6/graphrag/08_serving.md로 이동

---

## 6. 평가 전략

> §6 평가 전략 → 03.graphrag/results/implement_planning/separate/v6/graphrag/09_evaluation.md로 이동