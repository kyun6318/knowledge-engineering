...
> - [C-2] compute_industry_code_match() 코드 타입 혼용 수정: candidate 측을 INDUSTRY_SUBCATEGORY(workcondition.industryCodes)로 변경
...
```python
def compute_industry_code_match(company_industry_codes, candidate_industry_codes):
    """
    [v13 C-2] 수정: company 측과 candidate 측 모두 INDUSTRY 계열 코드로 통일.

    company_industry_codes: overview.industry_codes (INDUSTRY 타입, 3depth)
    candidate_industry_codes: workcondition.industryCodes (INDUSTRY_SUBCATEGORY 타입, 2depth)
                              또는 PastCompanyContext.industry_code (INDUSTRY 타입, 3depth)

    비교 전략:
    - 동일 코드 체계(INDUSTRY) 내에서 계층적 비교 수행
    - candidate 측이 INDUSTRY_SUBCATEGORY(2depth)인 경우, company의 INDUSTRY(3depth)를
      상위 2depth로 올려서 비교
    ...
    for c_code in company_industry_codes:
        ...
        if not company_detail:
            continue
        for r_code in candidate_industry_codes:
            # candidate 측은 INDUSTRY_SUBCATEGORY(2depth) 또는 INDUSTRY(3depth)
            candidate_detail = lookup_common_code(
                type="INDUSTRY_SUBCATEGORY", code=r_code
            )
            if not candidate_detail:
                # INDUSTRY(3depth)로 재시도
                candidate_detail = lookup_common_code(type="INDUSTRY", code=r_code)
            if not candidate_detail:
                continue

            # 3depth 일치: company INDUSTRY == candidate INDUSTRY
            if c_code == r_code:
                best_bonus = max(best_bonus, 0.25)
            # 2depth 일치: 동일 INDUSTRY_SUBCATEGORY
            elif company_detail.sub_code == candidate_detail.sub_code:
                best_bonus = max(best_bonus, 0.15)
            # 1depth 일치: 동일 INDUSTRY_CATEGORY
            elif company_detail.group_code == candidate_detail.group_code:
                best_bonus = max(best_bonus, 0.05)
    return best_bonus
```

> **[v13 C-2]** v12까지 candidate 측에 `JOB_CLASSIFICATION_SUBCATEGORY`(직무 분류 코드)를 사용했으나, 이는 산업 코드(`INDUSTRY`)와 다른 코드 체계이므로 직접 비교가 불가능하다. v13에서는 candidate 측도 `workcondition.industryCodes`(INDUSTRY_SUBCATEGORY) 또는 `PastCompanyContext.industry_code`(INDUSTRY)를 사용하여 동일 코드 체계 내에서 비교한다. 단, candidate의 industryCodes는 66% 빈배열이므로 PastCompanyContext 역참조(00_data_source_mapping §3.4)를 보조 소스로 활용한다.
