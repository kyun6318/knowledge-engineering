# 통합 Graph 스키마 v14 — 통합판

> v4 원본에 A2(Industry 노드 정의), A5(Company 간 관계 제외 명문화)를 통합.
>
> 작성일: 2026-03-10 | 현재 유효 버전: **v14** | 기준: v4 Graph Schema + v4 amendments (A2, A5) + v12 데이터 분석 v2.1
>
...
- **단방향 동기화**: 원본 DB → Neo4j + BigQuery (양방향 동기화 불필요)
- **Neo4j는 읽기 전용**: 매핑 결과(MAPPED_TO)만 Neo4j → BigQuery로 역방향 적재
...
def sync_to_neo4j(change_type: str, records: List[Dict]):
    ...
    """
    # MERGE 문으로 upsert (존재하면 업데이트, 없으면 생성)
    # 예: MERGE (p:Person {person_id: $pid}) SET p.name = $name, ...
    pass
```
