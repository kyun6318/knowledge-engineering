# CompanyContext v19 — 통합판

> v4 원본에 A3(CompanyTalentSignal 제외 명문화), A6(structural_tensions Taxonomy + Tier ceiling 예외)를 통합.
>
> 작성일: 2026-03-10 | 현재 유효 버전: **v19** | 기준: v4 CompanyContext + v4 amendments (A3, A6) + v12 데이터 분석 v2.1
>
> **v19 변경** (2026-03-11): 내용 변경 없음 (버전 헤더 동기화)
>
> <details><summary>v17~v18 변경 이력</summary>
>
> **v18 변경** (2026-03-10): 내용 변경 없음 (버전 헤더 동기화)
>
> **v17 변경** (2026-03-10): v16 리뷰 피드백 반영
> - [R-1] vacancy.scope_type → vacancy.hiring_context 명칭 변경 (Candidate scope_type과의 용어 충돌 해소)
>
> **v16 변경** (2026-03-10): v15 리뷰 피드백 반영
> - [S-6] §2.3 동일 기업 다중 공고 처리 원칙 추가
> - [U-8] §2.4 CompanyContext 재생성 조건 정의
> - [O-3] §2.2 structural_tensions 상세 정의에 "v1.1+ 상세화" 표시 추가
>
> <details><summary>v13~v15 변경 이력</summary>
>
> **v15 변경** (2026-03-10): 내용 변경 없음 (버전 헤더 동기화)
>
> **v14 변경** (2026-03-10): v13 리뷰 피드백 반영
> - [X-1] JSON 예시의 industry_code를 NICE 코드("J63112") → code-hub INDUSTRY 코드로 수정
> - [X-3] §1 Tier 정의에 Company T1(JD)과 Candidate T1(이력서) 상한 차이 명시
>
> <details><summary>v13 변경 이력</summary>
>
> **v13 변경** (2026-03-10): v12 데이터 분석 v2.1 결과 반영
> - industry_code: code-hub INDUSTRY 코드를 primary로 채택 (NICE는 보조)
> - vacancy 매핑: job-hub 구조화 필드 활용 강화
> - job-hub 필드 실측 분석은 Phase 4-1에서 수행 예정
>
> </details>
>
> </details>
...
> **[v14] 참고**: 본 문서의 T1(JD) 상한 0.80과 02_candidate_context의 T1(이력서) 상한 0.85는 서로 다른 데이터 소스에 대한 상한이다. JD는 채용 광고 특성상 과장/누락 가능성이 있어 이력서보다 낮은 상한을 적용한다.
...
**구현 원칙**: CompanyContext는 `job_id`별로 하나씩 생성하되, 동일 `company_id`의 공고들은 company_profile, stage_estimate, operating_model, structural_tensions를 **캐싱하여 재사용**한다. 이를 통해 동일 기업의 100개 공고에 대해 기업 수준 데이터(NICE 조회, stage 추정 등)를 1번만 수행한다.
...
> **[v16] v1.1+ 상세화 대상**: 아래 taxonomy 8개와 배타성 가이드, related_tensions 체계는 v1에서 70%+ null이 예상되므로, **v1에서는 tension_type 라벨만 사용하고 배타성 가이드/related_tensions는 v1.1 크롤링 활성화 후 적용한다.** v1 구현 시 tension_type enum과 기본 description만 구현하면 충분하다.
...
#### 2.4 CompanyContext 재생성 조건 [v16 신규]
...
| 크롤링 데이터 갱신 (v1.1+) | domain_positioning, structural_tensions 재생성 | 크롤링 완료 이벤트 |
| 공고 마감 | CompanyContext 아카이빙 (활성에서 제외) | job-hub 공고 상태 변경 감지 |

**재생성 우선순위**: 공고 수정 > 공고 마감 > NICE 갱신 > 크롤링 갱신

> **[v16]** CandidateContext의 재생성 조건은 `02_candidate_context.md §2.9`를 참조.
