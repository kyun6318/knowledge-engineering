# GraphRAG vs Vector Baseline 비교 실험 계획 v19

> v4 amendments A7(GraphRAG vs Vector 비교 실험 계획)을 독립 문서로 확장.
>
> 작성일: 2026-03-08 | 현재 유효 버전: **v19** | 기준: v4 amendments A7 + v7 Vector baseline 구체화
>
> **v19 변경** (2026-03-11): 내용 변경 없음 (버전 헤더 동기화)
>
> <details><summary>v18 변경 이력</summary>
>
> **v18 변경** (2026-03-10): 내용 변경 없음 (버전 헤더 동기화). 본 문서는 실험 설계 문서로, 온톨로지 설계와 분리된 구현/검증 참조 문서이다.
>
> </details>
>
> **v16 변경** (2026-03-10): v15 리뷰 피드백 반영
> - [O-7] §10 가상 실험 데이터를 결과 스키마 정의로 축소
>
...
> - [U-14] §1 후보 풀 ~500명 선정 기준 명시
> - [U-16] §5 B' LLM Reranking 시 LLM에게 제공하는 정보 범위 명확화
>
> <details><summary>v8~v11 변경 이력</summary>
>
> **v8 변경** (2026-03-08):
...
>
> **v9 변경** (2026-03-08):
> - [M-2] Step 6에 Krippendorff's alpha < 0.6 대응 방안 추가
> - [M-4] Step 2 처리량 표기를 조건부 표기로 교정
>
> **v10 변경** (2026-03-08):
> - [N-1] 사전 검정력 분석(power analysis) 섹션 추가 (1.1절)
> - 성공 기준에 효과 크기(Cohen's d)를 공동 주 판단 기준으로 격상
>
> **v11 변경** (2026-03-09):
> - 내용 변경 없음. v11의 code-hub 기반 구조화 매칭은 GraphRAG(A) 내부 개선이므로 실험 설계에 영향 없음
> - 단, Step 7 피처별 기여도 분석에서 code-hub 기반 매칭의 기여도를 별도 로깅할 것을 권장
...
| **후보 풀** | 동일 후보 풀 ~500명 (아래 선정 기준 참조) [v14] |
...
### 1.2 사전 검정력 분석 (Power Analysis) [v10]
...
> [v16] 가상 예시 데이터를 제거하고 결과 스키마만 유지. 실험 완료 후 이 스키마에 맞춰 실제 데이터를 기록한다.
...
> `method_key`: `method_A_graphrag`, `method_B_vector`, `method_B_prime_vector_llm`
> `comparison_key`: `paired_t_test_a_vs_b`, `paired_t_test_a_vs_b_prime`
