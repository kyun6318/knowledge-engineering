# MappingFeatures v4 — 구현 가능 재정의

> v3에서 미정의였던 **각 피처의 계산 로직**을 pseudo-code 수준으로 정의하고,
> CompanyContext / CandidateContext의 필드 가용성에 따른 **graceful degradation** 전략을 명시.
>
> 작성일: 2026-03-08 | 기준: v3 Context Overview + v4 CompanyContext/CandidateContext
...
def compute_stage_match(company_ctx, candidate_ctx):
    ...
    STAGE_SIMILARITY = {
        ("GROWTH", "GROWTH"): 1.0,
        ("GROWTH", "EARLY"): 0.5,    # 유사하지만 동일하지 않음
        ("GROWTH", "SCALE"): 0.4,
        ("SCALE", "SCALE"): 1.0,
        ("SCALE", "GROWTH"): 0.5,
        ("EARLY", "EARLY"): 1.0,
