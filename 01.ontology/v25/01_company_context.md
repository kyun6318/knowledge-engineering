...
> estimate_stage() pseudo-code → 02.knowledge_graph/results/extraction_logic/v18/03_prompt_design.md 참조
...
| `seniority_confidence` | number | JD | LLM + Rule | [v25] seniority 판정의 confidence (0.0~1.0). JD에 직급이 명시되면 0.85, 추론 시 0.50~0.70 |
...
> **[v25]** "v3 표현"은 초기 설계 단계(온톨로지 v3)에서 사용한 약식 표기로, 역사적 맥락으로만 참조한다.
...
> **[v21] v1 operating_model 추출 ROI 검토**: operating_model의 3개 facet 모두 confidence 기대치가 0.30~0.60이고, culture_fit(가중치 0.10) 매칭에만 사용되며, work_style_signals가 v1에서 대부분 null이므로 culture_fit 자체가 INACTIVE다. **v1에서 operating_model 추출에 LLM 토큰을 투입하는 ROI가 낮다.** v1에서는 구조화 필드(§2.5의 always_hire, work_schedule_option_types 등)에서 추출 가능한 시그널만 수집하고, LLM 기반 facet 추출은 생략하는 것을 권장한다. 절감된 토큰을 Outcome/SituationalSignal 품질 향상에 투입. v1.1(크롤링 보강) 이후 LLM 기반 facet 추출 활성화를 권장한다.

> extract_facet() pseudo-code + 키워드 → 02.knowledge_graph/results/extraction_logic/v18/03_prompt_design.md 참조
...
> §2.4 재생성 조건 → 03.graphrag/results/implement_planning/separate/v8/shared/regeneration_policy.md로 이동
...
interface Evidence {
  ...
  extraction_method?: string;  // [v22] "llm" | "rule_keyword" | "lookup_nice" (모델 독립적 추상화)
}
