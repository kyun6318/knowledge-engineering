...
- v1은 파이프라인이 ‘돈다’ + 산출물이 ‘서빙된다’ + DS/MLE가 ‘소비할 수 있다’까지
- v2는 정확도/표현력(챕터 세분화)과 커버리지(누락 보강) 확장을 모듈 단위로 붙이는 것

### 11.1 v1 MVP

- 목표
    - GraphRAG로 CompanyContext / CandidateContext / MappingFeatures를 생성
    - evidence/신뢰도/누락을 운영 규칙대로 기록
    - DS/MLE 랭킹 파이프라인에 연동(Join/API) 가능
    - 최소 평가(오프라인)로 “쓸만한지” 확인

**v1에서 반드시 포함할 “최소 챕터/피처”**

- CompanyContext:
    - stage(변곡점 라벨 1개) + vacancy_scope_type(0→1/1→10/reset)
    - operating_model facet 3개만(speed/autonomy/process) + narrative_summary
    - structural_tension은 타입 3개 중 하나로만(예: tech_debt_vs_features / speed_vs_reliability / founder_vs_pro_mgmt) — 없으면 Unknown
- CandidateContext:
    - experiences 1개 이상, situational_signals 2~3개(스케일/전환/레거시 등) + role_evolution 1개
    - PastCompanyContext: stage/scale/domain 팩트만(가능한 경우에만)
- MappingFeatures:
    - stage_transition_match, vacancy_fit, culture_fit(3facet), domain_positioning_fit(가능하면)
    - evidence_bundle는 회사/후보 각각 최소 1개 span 필수
    - v1의 원칙: “있을 때만 쓴다”, “근거 없으면 Unknown”.
...
- 산출물
    - 산출물 스키마 확정(Company/Candidate/MappingFeatures) + versioning/evidence 규칙
    - DS/MLE 소비 인터페이스 확정(테이블 조인 vs API)
    - 입력 소스 계약(내부 기업정보/크롤링/NICE, 자사/LinkedIn 이력서)
- Done
    - 스키마 문서 + 샘플 JSON 20건 생성 가능(더미로라도)

**Phase 1 — GraphRAG 파이프라인 “동작”**

- 산출물
    - GraphRAG indexing 파이프라인(Company corpus, Candidate corpus)
    - community report 생성 + evidence span 추출
    - CompanyContext/CandidateContext v1 생성(배치)
- Done
    - 샘플 규모(예: 회사 100/후보 1k)에서 end-to-end 생성 성공
    - 모든 claim에 evidence/source_type/confidence/extracted_at 기록

**Phase 2 — MappingFeatures + Serving + Integration**

- 산출물
    - MappingFeatures 생성(후보 리스트 입력받아 batch 생성)
    - 서빙(테이블/파일 or API)
    - 관측성(성공률/실패원인/처리량/latency) + 런북
    - 최소 오프라인 평가(DS+MLE)
- Done
    - DS/MLE 파이프라인이 mapping을 실제로 붙여서 1회 실험 가능
    - “컨텍스트 on/off” ablation 리포트 1회

### 11.3 v2 확장 전략

v2는 “더 똑똑하게”가 아니라 각 모듈이 추가하는 ‘새로운 피처’와 ‘새로운 evidence 생산’이 명확해야 함.

**v2 모듈 후보(우선순위 추천)**

1. StageTransitionContext 고도화
    - A→B, PMF→Scale, 효율화, 글로벌 확장 등 챕터 taxonomy 확장
    - PastCompanyContext 역산 정확도/커버리지 확대
2. CultureContext 확장(8 facets 전체)
    - v1 3facet → v2 8facet
    - 광고성 필터 강화 + proxy 규칙 고도화
3. ImpactContext
    - outcomes(지표/스케일/팀 성장) 추출 강화
    - 성과 근거 span + 숫자 extraction
4. RiskConstraintContext
    - 규제/레거시/품질 요구를 체계화
    - NICE/내부 기업정보 기반 prior 보정 강화
5. Closed-loop Enrichment
    - missing_fields 기반 타겟 질문 생성/수집/업데이트 파이프라인

### v1 vs v2 분리 기준(체크리스트)

**v1**

- 파이프라인이 돌아야만 다음이 가능한 것(인덱싱/리포트/서빙/연동)
- 최소 피처라도 있어야 DS/MLE가 “써보고” 평가 가능한 것

**v2**

- 피처 차원을 늘리는 것(3facet→8facet, taxonomy 확장)
- 커버리지를 올리는 것(누락 보강, 질문 루프)
- 정교한 품질 향상(정량 추출/추론 개선)