# MappingFeatures v19 — 통합판

> v4 원본에 A4(STAGE_SIMILARITY 전체 매트릭스 + 캘리브레이션), A1(F5 scope_type 변환)을 통합.
>
> 작성일: 2026-03-11 | 현재 유효 버전: **v19** | 기준: v4 MappingFeatures + v4 amendments (A1, A4) + v12 데이터 분석 v2.1
>
> **v19 변경** (2026-03-11): v18 리뷰 피드백 반영
> - [S-2] §4.2 freshness_weight의 MappingFeatures 적용 지점 명시 (overall_match_score 산출 후 랭킹 점수에 곱셈 적용)
>
> <details><summary>v18 변경 이력</summary>
>
> **v18 변경** (2026-03-10): v17 리뷰 피드백 반영
> - [R-6] §2 F1 STAGE_SIMILARITY 비대칭의 명시적 근거 추가
> - [R-7/U-7] §2 F3 industry_code 비교를 `[:3]` 슬라이싱에서 `lookup_common_code` group_code 조회로 수정
> - [R-9/O-7] §4 overall_match_score 기본 계산을 단순 가중 평균으로 변경 (이중 감쇠는 v1 파일럿 후 선택적 도입)
> - [R-5] §7.3을 피처별 ACTIVE 전망의 정본으로 확정
>
> </details>
>
...
> - [R-1] §2 F2 vacancy_fit에서 scope_type → hiring_context 명칭 변경 반영
> - [R-3] §2 F2 하단에 Outcome 활용 방침 명시 (v1에서 매칭 피처 미사용 이유와 v2 계획)
> - [R-6] §2 F4 하단에 v1 포함 근거 명확화
>
> **v16 변경** (2026-03-10): v15 리뷰 피드백 반영
> - [S-5] §0.1 v1 매칭 범위를 EXPERIENCED로 제한 명시, v2 신입 매칭 로드맵 추가
> - [S-3] §2 F2 REPLACE 공고 매칭 전략 보완 (role_fit 가중치 재분배)
> - [S-7] §2 F3/§4 스킬 매칭 시 SOFT_SKILL 제외 규칙 명시
> - [S-4] §4.1 overall_match_score 분포 모니터링 기준 추가
>
> </details>
>
> <details><summary>v8~v15 변경 이력</summary>
>
> **v15 변경** (2026-03-10): 내용 변경 없음 (버전 헤더 동기화)
>
> **v14 변경** (2026-03-10): v13 리뷰 피드백 반영
> - [X-4] F3 domain_fit의 industry code 매칭 로직을 정본으로 확정 (00_data_source_mapping §4.1은 데이터 소스 참조용)
> - [U-9] §4 FEATURE_WEIGHTS 설정 근거 및 캘리브레이션 계획 명시
> - [O-8] F5 ROLE_PATTERN_FIT에 캘리브레이션 계획 추가
>
> <details><summary>v8~v13 변경 이력</summary>
>
> **v13 변경** (2026-03-10): F1~F5 실측 ACTIVE 비율 보정, F3 industry_code_match v13 C-2 반영
>
> **v8 변경** (2026-03-08): [M-2] confidence 이중 감쇠 설계 의도 주석 추가
>
> </details>
...
**v1에서 MappingFeatures 계산 대상은 `career_type = "EXPERIENCED"` 후보로 제한한다.**
...
def compute_stage_match(company_ctx, candidate_ctx):
    ...
    # 설계 근거 [v18 R-6 명시적 근거 추가]:
    # - 대각선(동일 stage) = 1.0
    # - 비대칭 원칙: "상위 단계 경험자가 하위 단계에 적응하기 더 쉽다"
    #   GROWTH→EARLY(0.50): 성장기 경험자는 초기 환경의 불확실성을 이해하며,
    #     리소스 제약 하에서의 의사결정 경험을 일부 보유. 적응 가능성 중간.
    #   EARLY→GROWTH(0.30): 초기 경험만 있는 사람은 성장기의 프로세스 구축,
    #     대규모 조직 운영, 스케일링 의사결정 경험이 부족. 적응 난이도 높음.
    # - SCALE→GROWTH(0.50): 대규모 조직에서 성장기 조직으로 이동 시
    #     속도감/민첩성 적응은 가능하나 리소스 감소에 대한 적응 필요.
    #   GROWTH→SCALE(0.40): 성장기 경험자가 대규모 조직의 복잡도/거버넌스 적응은 상대적으로 어려움.
    # - MATURE↔SCALE 양방향 0.45: 두 단계 모두 대규모 조직으로 문화적 유사성 높음.
    # - 원거리(EARLY↔MATURE) 0.10: 조직 규모/문화/의사결정 방식이 근본적으로 다름.
...
### F2 보충: Outcome 활용 방침 [v17 R-3 신규]

**v1에서 Outcome은 MappingFeatures 매칭 피처로 사용하지 않는다.** 이는 의도적 설계이다.
...
> **[v14] 정본 선언**: F3 domain_fit의 industry code 매칭 로직은 본 문서가 정본이다. 00_data_source_mapping §4.1은 데이터 소스 관점의 참조 정보만 기술한다.
...
def compute_domain_fit(company_ctx, candidate_ctx):
    ...
    # [v18 R-7/U-7] industry_code[:3] 슬라이싱 제거 → code-hub lookup 기반 계층 비교로 변경.
...
> **[v17] F4 v1 포함 근거 [R-6]**: F4 culture_fit의 v1 ACTIVE 비율이 <10%임에도 v1 피처 목록에 포함하는 이유:
> 1. **프레임워크 완성도**: 5개 피처 구조를 v1에서 확정하여 v1.1/v2에서 피처 추가 없이 활성화만으로 확장 가능. v1에서 4개로 시작하고 v2에서 5개로 늘리면 overall_match_score 가중치 체계가 변경되어 기존 매핑과의 연속성이 깊어짐
> 2. **가중치 최소화**: 0.10 가중치(5개 중 최소)로 INACTIVE 시 다른 피처에 자연 재분배되므로, 포함해도 매칭 품질에 부정적 영향 없음
> 3. **조기 데이터 수집**: ACTIVE 10% 케이스에서도 work_style_signals 추출 품질을 모니터링하여 v1.1 보강 전략 수립에 활용
> 4. **v1에서 제거 시 리스크**: v2에서 피처를 "추가"하면 A/B 테스트 등 기존 실험 결과의 연속성이 깨짐. 처음부터 포함하고 INACTIVE→ACTIVE 전환이 운영상 더 안전
...
# [v14] ROLE_PATTERN_FIT 캘리브레이션 계획:
...
def compute_overall_score(features, use_double_dampening=False):
    ...
    if use_double_dampening:
        # [v8 원본] confidence 가중 스코어 (의도적 이중 감쇠)
        ...
        # [v18 기본] 단순 가중 평균 — score만 사용, confidence는 메타 정보로 보고
        return sum(
...
### 4.1 overall_match_score 분포 모니터링 [v16 신규]
...
> **[v16] REPLACE 공고 처리**: REPLACE 공고에서 vacancy_fit이 INACTIVE가 되면, 해당 가중치(0.30)가 나머지 활성 피처에 재분배된다. 특히 role_fit(기존 0.15)이 재분배 후 실질 가중치가 크게 상승하므로, REPLACE 공고에서는 역할 적합도가 매칭의 핵심 기준이 된다. 이는 "충원" 목적의 공고에서 역할/시니어리티 매칭이 가장 중요하다는 도메인 직관과 일치한다.

### 4.2 freshness_weight 적용 규칙 [v19 S-2 신규]
