...
def extract_signals(experience_text, company_info=None):
    ...
    # [v19 S-1] 프롬프트에 경계 가이드와 few-shot 예시를 인라인으로 포함
    prompt = f"""
...
{SIGNAL_TAXONOMY}
    ...
    [모호한 조합 판정 기준 - 아래 규칙을 반드시 따르세요]
    - SCALE_UP vs TEAM_SCALING: 조직 전체의 성장이면 SCALE_UP, 특정 팀/부서 확대이면 TEAM_SCALING
      예) "회사가 급성장하며 직원 50->200명" -> SCALE_UP / "백엔드 팀 5->20명 확장" -> TEAM_SCALING
    - NEW_SYSTEM_BUILD vs TECH_STACK_TRANSITION: 기존 시스템 없이 새로 구축이면 NEW_SYSTEM_BUILD, 기존 시스템에서 전환이면 TECH_STACK_TRANSITION
      예) "결제 시스템 0->1 구축" -> NEW_SYSTEM_BUILD / "모놀리스->MSA 전환" -> TECH_STACK_TRANSITION
    - SCALE_UP vs EARLY_STAGE: 급성장 결과에 초점이면 SCALE_UP, 초기 불확실성/소규모에 초점이면 EARLY_STAGE
      예) "시리즈 B 이후 트래픽 10배" -> SCALE_UP / "3번째 직원으로 합류" -> EARLY_STAGE
    - TEAM_BUILDING vs TEAM_SCALING: 없던 팀을 새로 만들었으면 TEAM_BUILDING, 기존 팀을 확대했으면 TEAM_SCALING
      예) "데이터팀 신설 0->5명" -> TEAM_BUILDING / "기존 팀 5->20명" -> TEAM_SCALING
    - TURNAROUND vs LEGACY_MODERNIZATION: 사업/조직 방향 전환이면 TURNAROUND, 기술/시스템 개선이면 LEGACY_MODERNIZATION
      예) "피봇 후 B2B 전환" -> TURNAROUND / "레거시 리팩토링" -> LEGACY_MODERNIZATION

    위 모호한 조합에 해당하면 판정 기준에 따라 하나만 선택하세요.
    모호하지 않은 경우, 2개 이상의 signal이 명확한 근거와 함께 해당되면 복수 선택을 허용합니다.
...
> **전제 조건**: Career[] 배열은 `period.start` 기준 **시간 역순** (최근 -> 과거)으로 정렬되어야 한다. resume-hub에서 조회 시 `ORDER BY period.started_on DESC`를 적용한다. 시간순이 보장되지 않으면 role_evolution 추출이 부정확해질 수 있다.
> 
...
> CompanyContext의 재생성 조건은 `01_company_context.md §2.4`를 참조.
> 
...
GraphDB Ideation에서 정의한 노드/관계를 CandidateContext와 매핑.

### 노드 매핑

| GraphDB 노드 | CandidateContext 필드 | 속성 |
| --- | --- | --- |
...
situational_signals를 **공유 가능한 노드**로 분리하여 “같은 상황을 경험한 후보”를 그래프 탐색으로 찾을 수 있게 한다.
...
[resume-hub DB]
    ...
    │   ├─ Resume.userUpdatedAt -> freshness_weight
    │   ├─ Career[] -> company, period (DATERANGE -> duration_months 직접 계산)
    ...
    │   ├─ Education[] -> education_level (schoolType 진실 소스)
    │   └─ Certificate[] -> type 변환 후 codehub 조회
    │
    ...
    │
    ├─[3] LLM 추출 (전체 커리어)
    │   ├─ CareerDescription.description (16.9%) -> outcomes 1차 추출
    ...
    │   ├─ Career.companyName -> job-hub 역참조 (confidence 0.75)
