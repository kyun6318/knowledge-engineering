...
| T3 | LinkedIn 프로필 | 0.65 | **BrightData 2.0M 프로필 확보**, AI 표준화 경력 완료. 상세: `00.datamodel/summary/v3.md §2.2` | `linkedin` |
...
interface SituationalSignal {
  ...
  confidence: number;
  evidence: Evidence[];          // [v25] 복수 근거 허용
}
```
...
> extract_signals() pseudo-code → 02.knowledge_graph/results/extraction_logic/v18/03_prompt_design.md 참조
...
> build_past_company_context() pseudo-code → 02.knowledge_graph/results/extraction_logic/v18/03_prompt_design.md 참조
...
### 2.7.1 SIE 모델 보완 (v3 신규)

SIE(GLiNER2/NuExtract 1.5) 모델은 CareerDescription FK 부재 제약을 부분적으로 해소한다:
- GLiNER2의 Start/End Span을 활용하여 원문 내 경력별 구간 자동 분리
- career별 귀속이 가능해져 Outcome/SituationalSignal 추출 정확도 향상 기대
- **적용 범위**: workDetails(~56%) 텍스트에서 경력-프로젝트-성과 구조 직접 추출

> 상세 모델 설계: `00.datamodel/model/sie-model.md`
> v3 데이터 분석: `00.datamodel/summary/v3.md §3`
...
> extract_role_evolution() pseudo-code → 02.knowledge_graph/results/extraction_logic/v18/03_prompt_design.md 참조

### 2.9 Context 재생성 조건

> §2.9 재생성 조건 → 03.graphrag/results/implement_planning/separate/v8/shared/regeneration_policy.md로 이동
...
> §4 GraphDB 엔티티 매핑 → 04_graph_schema.md §1-§2에 통합. Neo4j 구현 상세는 03.graphrag/results/implement_planning/separate/v8/graphrag/07_neo4j_schema.md 참조.
