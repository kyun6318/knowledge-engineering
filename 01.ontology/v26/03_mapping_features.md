...
> 작성일: 2026-03-14
>
> v26 변경: [A-3] REPLACE 비율 사전 분석 요구사항 Phase 0 전 필수로 추가
    ...
    for exp in candidate_ctx.experiences:
        ...
        if pcc and pcc.estimated_stage_at_tenure:
            stage_experiences.append({
                ...
                "pcc_confidence": pcc.confidence
            })

    if not stage_experiences:
        return inactive("no past_company_context with stage")

    # [v7] [v1-PILOT] STAGE_SIMILARITY 전체 4x4 매트릭스 (A4 적용)
    # 아래 값은 전문가 판단 기반 초기값이며, Phase 0 파일럿 후 [v1-PRODUCTION]으로 확정한다.
...
VACANCY_SIGNAL_ALIGNMENT = {
    "BUILD_NEW": {
        ...
        "moderate": ["TEAM_BUILDING", "TECH_STACK_TRANSITION", "GLOBAL_EXPANSION"],
        "weak": ["SCALE_UP"]
    },
    "SCALE_EXISTING": {
        "strong": ["SCALE_UP", "TEAM_SCALING"],
        "moderate": ["LEGACY_MODERNIZATION", "ENTERPRISE_TRANSITION", "GLOBAL_EXPANSION"],
        "weak": ["NEW_SYSTEM_BUILD", "MONETIZATION"]
    },
    "RESET": {
...
# [v24] 14개 taxonomy 중 VACANCY_SIGNAL_ALIGNMENT 미배정 시그널 확인:
# 모든 14개 시그널이 최소 1개 hiring_context에 배정되어 있어야 한다.
# GLOBAL_EXPANSION: BUILD_NEW(moderate), SCALE_EXISTING(moderate)
# MONETIZATION: SCALE_EXISTING(weak)
# OTHER: 모든 hiring_context에서 의도적 제외 (범용 시그널)
...
def compute_vacancy_fit(company_ctx, candidate_ctx):
    ...
    if not alignment or vacancy_type == "REPLACE":
        ...
        #
        # [v26] A-3: REPLACE 비율이 전체 매칭 품질에 미치는 영향이 크므로,
        # Phase 0 PoC 착수 전에 반드시 아래 사전 분석을 완료해야 한다.
        # (§4.1 REPLACE 공고 비율 사전 분석 참조)
    ...
    if strong_matches:
        ...
        base_score = 0.35
    # [v23] "no matching signals" 설계 근거:
    # - INACTIVE가 아닌 ACTIVE(0.15)를 반환하는 이유:
    #   signals가 아예 없는 경우(L234-235)는 INACTIVE이지만,
    #   signals는 있으나 alignment 매칭이 안 되는 것은 "불일치 확인"이므로
    #   가중치 재분배 대상이 아닌 저점수 매칭으로 처리한다.
    # - 0.15 baseline: strong(0.85), moderate(0.60), weak(0.35) 대비
    #   "확인된 불일치" 수준. Phase 0에서 이 값의 적정성을 검증한다.
    else:
...
### [v22] F2 vacancy_fit confidence 분포 예상

CareerDescription FK 부재(02/01_extraction_pipeline.md §3.2)에 따른 감쇠율이 F2에 미치는 영향:

| 데이터 보유 상태 | 비율 | 감쇠 | F2 예상 confidence |
|---|---|---|---|
| CareerDescription 보유 | 16.9% | 없음 | 0.60~0.85 (정상) |
| SelfIntroduction only | 53.3% | 0.5x | 0.30~0.43 (Low) |
| 둘 다 미보유 | 29.8% | — | INACTIVE |

> **[v24]** 산출식: SelfIntro only = (100% - 16.9%) × 64.1% ≈ 53.3%, 둘 다 미보유 = (100% - 16.9%) × (100% - 64.1%) ≈ 29.8%. 독립 가정.

**영향**: F2가 ACTIVE인 경우에도 53.3%가 SelfIntroduction 경유로 인해 confidence가 0.30~0.43 수준이다. 이는 overall_match_score에서 F2의 실질 기여도를 낮추어, vacancy_fit 가중치 0.30이 명목상보다 실효성이 떨어질 수 있다.

**Phase 0 실측 과제**: PoC 20건에서 F2의 실제 confidence 분포를 측정하고, SelfIntroduction 경유 시 0.5x 감쇠가 적절한지 검증한다. 감쇠율이 과도하면 0.6x~0.7x로 조정을 검토한다.
...
| CareerDesc 보유 | SelfIntro 보유 | Outcome 추출 | 감쇠 | Neo4j 적재 |
|----------------|---------------|-------------|------|-----------|
| O (16.9%) | O/X | O (CareerDesc 기반) | 없음 | 선택적 (v2 활용 대비) |
| X | O (64.1%) | O (SelfIntro 기반) | 0.5x confidence 감쇠 | 선택적 |
| X | X (35.9%) | X | — | X |

> 전체 Outcome 추출 가능 비율: 16.9% + (100-16.9%) x 64.1% ≈ **70.2%**. 단, 47.2%는 SelfIntro 경유로 confidence가 감쇠된 상태이다.
...
def compute_domain_fit(company_ctx, candidate_ctx):
    ...
    # [v24] embed() = text-embedding-005 (Vertex AI, 768d)
    # Chapter/Vacancy 임베딩과 동일 모델 사용 (01_extraction_pipeline.md Vector Index 참조)
    ...
    #
    # [v23] code-hub INDUSTRY 코드 계층 매핑:
    # | 계층         | code-hub 타입           | lookup_common_code 반환 필드    | 예시    | 저장 위치              |
    # |-------------|------------------------|-------------------------------|--------|----------------------|
    # | 1depth(대분류) | INDUSTRY_CATEGORY      | group_code                    | "I"    | Industry.category     |
    # | 2depth(중분류) | INDUSTRY_SUBCATEGORY   | code (= sub_code)            | "I37"  | Industry.industry_id  |
    # | 3depth(소분류) | INDUSTRY               | code                         | "I371" | Organization.industry_code |
    #
    # F3의 candidate_detail.sub_code는 lookup_common_code(type="INDUSTRY", code=X)의
    # 상위 SUBCATEGORY 코드(2depth)를 의미한다.
...
# [v24] speed와 autonomy가 동일 work_style 필드(autonomy_preference)에 매핑되는 설계 의도:
# v1에서 "빠른 실행"과 "높은 자율성"을 동일한 선호 축으로 간주한다.
# v2에서 culture_fit 활성화 시, speed를 별도 work_style 필드(예: iteration_speed_preference)로
# 분리하여 변별력을 높이는 것을 검토한다.
# experiment_orientation(WorkStyleSignals에 정의됨)에 매핑되는 facet은 현재 없으며, v2에서 추가 검토.
...
ALIGNMENT_LOGIC = {
    ...
    # [v24] 나머지 21개 조합(speed_mid/low, autonomy_high/mid/low, process_mid/low)은
    # v2 culture_fit 활성화 전까지 기본값 0.5로 fallback된다.
    # v2 활성화 시 전체 27개 조합 매트릭스를 Phase 5에서 완성한다.
}
...
ROLE_PATTERN_FIT = {
    ...
    # [v25] 미정의 조합은 기본값 0.50 반환 (.get() fallback)
}
...
def compute_role_fit(company_ctx, candidate_ctx):
    ...
    # [v23] experiences 정렬 규약: Data Contract은 period_start 오름차순이므로
    # 최신 경력은 배열 마지막 요소이다.
    latest_exp = candidate_ctx.experiences[-1] if candidate_ctx.experiences else None
    if latest_exp and latest_exp.scope_type != "UNKNOWN":
        # [v25] scope_type을 seniority로 변환: IC/LEAD/HEAD/FOUNDER/UNKNOWN 매핑
    ...
    # [v24] Confidence 전파 규칙 테이블(§2 하단)과 통일:
    # F5 confidence = min(seniority_source_confidence, role_evolution_confidence)
    # vacancy.seniority_confidence가 없으면 기본 상한 0.70 적용
    seniority_conf = getattr(company_ctx.vacancy, 'seniority_confidence', 0.70)
    confidence = min(
        seniority_conf,
        role_evo.confidence
    )
...
### [v23] Confidence 전파 규칙

엣지 레벨 confidence(예: `HAS_SIGNAL.confidence`)와 피처 레벨 confidence(예: `F2.confidence`)의 관계:

| 피처 | confidence 산출 규칙 |
|------|-------------------|
| F1 stage_match | `min(company.stage_confidence, best_match.pcc_confidence)` — 양쪽 소스 최소값 |
| F2 vacancy_fit | `min(vacancy.evidence[0].confidence, best_signal.confidence)` — 공고 근거와 시그널 최소값 |
| F3 domain_fit | `min(0.60, 0.40 + code_match_bonus)` — 임베딩 해석 한계로 보수적 상한 |
| F4 culture_fit | v1 INACTIVE (기본 0.5) |
| F5 role_fit | `min(vacancy.seniority_confidence ∥ 0.70, role_evolution_confidence)` — vacancy seniority confidence가 없으면 기본 상한 0.70 |

> 엣지 confidence는 해당 관계의 추출 신뢰도이고, 피처 confidence는 매칭 판단의 신뢰도이다. 피처 confidence는 관련된 엣지 confidence 중 **최소값**(보수적 전파)을 기본 전략으로 한다.
...
```json
{
  ...
  "features": {
    "stage_match": {
      ...
      "confidence": 0.55,
      "status": "ACTIVE",
      "matched_experience_id": "P_99999_ch0",
...
| REPLACE 공고 overall 분포 | role_fit 주도 (vacancy_fit INACTIVE) | role_fit도 INACTIVE -> overall 산출 불가 비율 10%+ | REPLACE 전용 fallback 피처 검토 (하단 [v22] [v24 통합] 참조) |
...
> **[v26] A-3: REPLACE 비율 사전 분석 — Phase 0 착수 전 필수**
>
> REPLACE 공고에서 vacancy_fit(가중치 0.30)이 INACTIVE가 되면 role_fit이 해당 가중치를 흡수하여, 매칭이 역할/시니어리티 단일 축으로 편중된다. v16에서 도입 이후 10개 버전간 REPLACE의 실제 비율을 측정한 적이 없다.
>
> **검증 필요 사항** (job-hub 데이터 조회):
> 1. `overview.descriptions`에서 hiring_context 키워드 빈도 분석 (BUILD_NEW/SCALE_EXISTING/RESET/REPLACE 관련 키워드)
> 2. 무작위 JD 100건에 대해 LLM으로 hiring_context를 분류하여 REPLACE 비율 실측
> 3. REPLACE 비율별 시나리오 영향도:
>    - 20% 미만: 현행 가중치 재분배로 충분
>    - 20~40%: role_fit 단독 지배 리스크 → REPLACE 전용 보조 피처(경력 연수 정확 매칭, 기술 스택 중첩도) 검토
>    - 40% 이상: Go/No-Go 의사결정 필요 → vacancy_fit 설계 자체 재검토
>
> **[v22] [v24 통합] REPLACE 공고 비율 및 활성 피처 정책**:
> - **비율 모니터링(Phase 0 최우선)**: JD의 hiring_context 분포를 실측하여 REPLACE 비율을 확인한다. REPLACE가 전체 JD의 30% 이상이면 vacancy_fit 가중치(0.30)의 대부분이 role_fit으로 흡수되어 전체 매칭 품질에 상당한 영향을 미칠 수 있다. 50%+일 경우 Go/No-Go 의사결정 항목에 포함한다. Phase 0에서 JD 100건 이상의 hiring_context 분포를 실측한다.
> - **REPLACE 전용 피처**: REPLACE 비율이 30%+ 시, 경력 연수 정확 매칭, 기술 스택 중첩도 등 REPLACE 전용 피처 도입을 Phase 2에서 검토한다.
> - **활성 피처 부족 시 low_confidence_flag**: 가중치 재분배 후 활성(ACTIVE) 피처가 2개 이하인 경우, `overall_match_score`를 산출하되 `low_confidence_flag: true`를 부여한다. 서빙 시 low_confidence_flag가 true인 매핑은 별도 처리(예: 결과 목록 하단 배치, UI에서 신뢰도 경고 표시)를 적용한다.

### 4.2 freshness_weight 적용 규칙

> §4.2 freshness_weight/ranking_score 서빙 구현 → 03.graphrag/results/implement_planning/separate/v8/graphrag/08_serving.md로 이동

---

## 5. DS/MLE 소비 인터페이스

> §5 BigQuery 서빙 스키마/SQL → 03.graphrag/results/implement_planning/separate/v8/graphrag/08_serving.md로 이동

---

## 6. 평가 전략

> §6 평가 전략 → 03.graphrag/results/implement_planning/separate/v8/graphrag/09_evaluation.md로 이동