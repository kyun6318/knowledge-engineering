...
>
> v26 변경: [A-1] company_id 키 미확정 → 검증 필요 사항 명시, [A-4] domain_depth 코드 체계 혼합 버그 수정, [D-8] Skill 데이터 resume 단위 제약 명시, 레이블 정리
...
| **ID 연결** | candidate_id는 resume-hub `SiteUserMapping.id`에 대응된다. company_id는 **v1 구현 전 검증 필수** — 아래 §2.1 참조 |
| **증분 보강** | DB 구조화 데이터를 기본 골격으로, LLM 추출 결과 및 크롤링 추가 데이터를 보강 레이어로 적용한다 |
| **실측 데이터 기반 설계** | fill rate, 품질 수치를 v3 데이터 분석 실측치로 처리하고 설계 결정은 실측 데이터에 근거한다 |
...
def resolve_industry_code(job_overview):
    ...
    # code-hub에서 코드 상세 조회
    # [v23] lookup_common_code 반환 필드 정의:
    # - code_detail.code: 조회한 코드 자체 (예: "I371")
    # - code_detail.detail_name: 코드 명칭 (예: "반도체 제조업")
    # - code_detail.group_code: 1depth 대분류 코드 (예: "I")
    # - code_detail.group_name: 대분류 명칭 (예: "제조업")
    # - code_detail.sub_code: 2depth 중분류 코드 (예: "I37") — INDUSTRY_SUBCATEGORY
    # 03_mapping_features.md F3의 계층 비교는 이 필드들을 사용한다.
...
> → 02.knowledge_graph/results/extraction_logic/v18/06_normalization.md로 이동
...
| --- | --- | --- |
| `company_id` | **미확정** — 후보: `job.user_ref_key` 또는 `job.workspace_id` | 기업 식별자. 동일 기업의 복수 공고를 묶는 키. **아래 검증 항목 참조** |
| `job_id` | `job.id` (VARCHAR 126) | 공고 식별자 |
| `company_name` | `work_condition.company_name` | 근무 기업명 |

> **[v26] company_id 키 선정 — v1 구현 전 검증 필수**
>
> v12에서 도입 이후 13개 버전간 미확정 상태로, 이 결정 없이는 Organization 노드 생성이 불가능하다.
>
> **후보 키 비교**:
>
> | 후보 키 | 설명 | 장점 | 리스크 |
> |---|---|---|---|
> | `job.user_ref_key` | 원본 사용자(기업회원) 참조키 | 기업회원 단위로 공고를 묶을 수 있음 | 동일 기업이 복수 회원으로 등록된 경우 분리됨 |
> | `job.workspace_id` | 비즈센터 Workspace ID | 비즈센터 단위로 묶어 기업 단위에 더 가까울 수 있음 | fill rate, 유니크 수 미확인 |
>
> **검증 필요 사항** (job-hub 데이터 조회):
> 1. `user_ref_key`와 `workspace_id` 각각의 유니크 수
> 2. 두 키 간 관계: 1:1인지, 1:N인지, N:M인지
> 3. 동일 `work_condition.company_name`에 대해 각 키가 몇 개로 분리되는지 (예: "삼성전자"에 대한 user_ref_key 수 vs workspace_id 수)
> 4. 각 키의 NULL/빈값 비율
>
> **결정 기준**: 동일 기업명에 대해 더 적은 수의 고유값을 생성하는 키가 Organization 노드 키로 적합하다. 두 키 모두 과도하게 분산되는 경우, `work_condition.company_name` 정규화 기반 키 생성을 대안으로 검토한다.
...
| 온톨로지 필드 | resume-hub 소스 | 추출 방법 | 실측 fill rate |
| --- | --- | --- | --- |
...
| `tech_stack` | `Skill` 테이블 (type=HARD, resume_id 기준) | Lookup (code-hub) | 38.3% (3,074,732 이력서). **[v26] 제약: Skill 테이블은 resume_id 기준이므로 career(경력) 단위가 아닌 이력서 전체 단위.** 복수 경력 보유 후보의 경우 특정 Chapter에서 어떤 Skill을 사용했는지 구분 불가 → USED_SKILL 엣지는 모든 Chapter에 동일 Skill 세트가 연결됨 (04_graph_schema.md §2.1 참조) |
| `scope_type` | `Career.positionTitleCode` -> `positionGradeCode` -> LLM | Rule + LLM | positionTitle **29.45%**, positionGrade **39.16%** |
| `scope_summary` | `Career.workDetails` 또는 `CareerDescription.description` | LLM | workDetails ~56%, careerDesc **16.9%**, **SIE 확장 가능** |
| `outcomes` | `CareerDescription.description` + `SelfIntroduction` | LLM | careerDesc **16.9%** (1차), selfIntro **64.1%** (2차), **SIE 보조** |
...
**[D5] SIE 모델 보조 추출 (v3 신규)**:

> SIE(Structure Information Extraction) 모델은 경력기술서/workDetails에서 경력-프로젝트-성과 계층 구조를 직접 추출한다.
> GLiNER2(주력): Span 기반, Hallucination 없음, confidence/threshold 개별 설정
> NuExtract 1.5(보조): 장문/혼합 언어 처리, 초기 데이터 레이블링용
>
> **적용 범위**: CareerDescription(16.9%) 한계를 workDetails(~56%)까지 확장하여 Outcome 커버리지 향상
> **온톨로지 매핑**: experience.achievements → Outcome 노드, project.tech_stack → Skill(USED_SKILL)
> 상세: `00.datamodel/model/sie-model.md`, `00.datamodel/summary/v3.md §3`
...
| 온톨로지 필드 | resume-hub 소스 | 추출 방법 | 실측 |
| --- | --- | --- | --- |
...
def extract_domain_depth_structured(resume):
    ...
    [v26] A-4 버그 수정: jobClassificationCodes(JOB_CLASSIFICATION_SUBCATEGORY)와
    industryCodes(INDUSTRY_SUBCATEGORY)는 서로 다른 코드 체계이므로 분리 집계한다.
    v11~v25에서 두 코드를 동일 Counter에 합산하여 잘못된 primary_domain이 산출되는 버그가 있었다.
    """
    from collections import Counter

    # 1. 직무 코드 집계 (JOB_CLASSIFICATION_SUBCATEGORY 체계)
    job_codes = []
    for career in resume.careers:
        if career.jobClassificationCodes:
            job_codes.extend(career.jobClassificationCodes)

    # 2. 산업 코드 집계 (INDUSTRY_SUBCATEGORY 체계) — 별도 Counter
    industry_codes = []
    ...
    # 직무 코드 기반 primary_domain (주력)
    result = {}
    if job_codes:
        job_counts = Counter(job_codes)
        primary_job_code, job_count = job_counts.most_common(1)[0]
        code_detail = lookup_common_code(
            type="JOB_CLASSIFICATION_SUBCATEGORY", code=primary_job_code
        )
        result = {
            "primary_domain": code_detail.sub_name if code_detail else primary_job_code,
            "domain_experience_count": job_count,
            "confidence": min(0.80, 0.50 + job_count * 0.10),
        }

    # 산업 코드 기반 보조 정보 (있을 때만 추가)
    if industry_codes:
        ind_counts = Counter(industry_codes)
        primary_ind_code, ind_count = ind_counts.most_common(1)[0]
        ind_detail = lookup_common_code(
            type="INDUSTRY_SUBCATEGORY", code=primary_ind_code
        )
        result["preferred_industry"] = ind_detail.detail_name if ind_detail else primary_ind_code
        result["preferred_industry_code"] = primary_ind_code

    # 직무 코드 없이 산업 코드만 있는 경우 (fallback)
    if not job_codes and industry_codes:
        result["primary_domain"] = result.get("preferred_industry", industry_codes[0])
        result["domain_experience_count"] = 0
        result["confidence"] = 0.30  # 산업 코드만으로 추정 → 낮은 confidence
...
| 온톨로지 필드 | resume-hub 소스 | 비고 |
| --- | --- | --- |
| `company_name` | `Career.companyName` | 99.96% fill rate (resume-hub), LinkedIn 933,923 고유 회사 교차 가능 |
...
def compute_freshness_weight(user_updated_at, use_smooth=False):
    ...
    [v23] v1 기본값: use_smooth = False (step 모드).
    smooth 모드 전환 조건: Phase 0 파일럿 50건에서 step/smooth 간
    Top-10 순위 변동이 10% 이상일 때 전환을 검토한다.
...
### 3.7 LinkedIn 외부 데이터 매핑 (v3 신규)

| 소스 | 온톨로지 활용 | 비고 |
| --- | --- | --- |
| `linkedin_experience_standardized.standardized_company` | Organization.name 교차 검증 | 933,923 고유 회사, 한/영 혼재 |
| `linkedin_experience_standardized.standardized_title` | Role 매핑 보조 | AI 표준화 완료 |
| `linkedin_experience_standardized.job_function` | Role.category 보조 | 직무 분류 |
| `linkedin_experience_standardized.seniority` | Chapter.scope_type 보조 | 직급 |
| `linkedin_experience_standardized.confidence` | 교차 검증 가중치 | High/Medium/Low |
| `brightdata_dump_raw_stage0b.education` | Person.education_level 교차 | 41.0% 보유 |

**LinkedIn ↔ resume-hub 교차 매핑 전략** (Phase 5):
1. Organization: LinkedIn company_id를 회사명 정규화 3차 패스로 활용
2. Role: LinkedIn AI 표준화 seniority/job_function으로 scope_type/Role 보강
3. 교차 검증: 동일 인물 매칭 시 confidence 보정 (02_candidate_context.md §0 교차 검증 규칙 적용)

> **제약**: LinkedIn 프로필과 resume-hub 이력서의 동일 인물 매칭은 이름+회사명+기간 조합으로 추정하며, 정확도 검증이 Phase 5에서 필요하다.
...
> §5 추출 파이프라인 → 02.knowledge_graph/results/extraction_logic/v18/01_extraction_pipeline.md 참조

---

## 6. 데이터 품질 및 coverage

> §6 데이터 품질/coverage 실측치 → 02.knowledge_graph/results/extraction_logic/v18/07_data_quality.md로 이동

---

## 7. 정규화 선행 과제

> §7 정규화 선행 과제 → 02.knowledge_graph/results/extraction_logic/v18/06_normalization.md로 이동

---

## 8. 구현 로드맵

> §8 구현 로드맵 → 03.graphrag/results/implement_planning/separate/v8/shared/implementation_roadmap.md로 이동
...
| code-hub `REGION` name | 6,858건 중 name 빈값 다수 — 지역 코드 정규화 시 name 필드 의존 불가, code 기반 매핑만 사용 |

**LinkedIn/BrightData 사용 불가 필드** (v3 신규):

| 필드 | 사유 |
| --- | --- |
| `location` | city 축약값, 채움률 28.6%, 완전 일치 0건 |
| `country_code` | 전부 KR (상수) |
| `recommendations_count` | 99.1% 무효 (-1) |
| `languages, certifications, honors` | 전부 채움률 10% 미만 |
| `about` | 채움률 17.2%, 비정형 장문 텍스트 |
...
> §10 LLM 비용 → 03.graphrag/results/implement_planning/separate/v8/shared/implementation_roadmap.md로 이동