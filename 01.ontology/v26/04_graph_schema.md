...
> 작성일: 2026-03-14
>
> v26 변경: [A-2] USED_SKILL 엣지 resume 단위 제약 명시 및 v1 전략 추가, [D-5] Vacancy.seniority_confidence 추가, [U-2] NEEDS_SIGNAL 엣지 생성 규칙 참조 추가
...
(:Vacancy {
  ...
  seniority_confidence: FLOAT, -- [v26] designation 기반 추정 시 상한 0.65, JD 명시 시 0.80. 03_mapping_features.md F5에서 참조
  team_context: STRING,
...
| `(:Chapter)-[:USED_SKILL]->(:Skill)` | 사용 기술 | - (아래 recency 결정 및 **resume 단위 제약** 참조) |
...
>
> **[v26] USED_SKILL 데이터 제약 — resume 단위 Skill**:
> resume-hub의 `Skill` 테이블은 `resume_id` 기준이며, `career_id` FK가 없다. 따라서 특정 경력(Career)에서 어떤 Skill을 사용했는지 구분이 불가능하다.
>
> **v1 전략**: 해당 이력서의 전체 HARD_SKILL을 **모든 Chapter에 동일하게** USED_SKILL 엣지로 연결한다. 이는 "Python을 A사에서만 사용했는데 B사 Chapter에도 연결"되는 과대 연결(over-linking) 문제를 야기하지만, v1에서 Skill은 MappingFeatures의 보조 피처(F3 domain_fit 보조)이므로 영향이 제한적이다.
>
> **v2 개선 계획**: (1) SIE 모델(GLiNER2)의 project.tech_stack 추출 결과를 활용하여 Career 단위 Skill 귀속 시도, (2) workDetails 텍스트에서 LLM으로 Skill-Career 매핑 추출, (3) 정확도 검증 후 Chapter별 차별화된 USED_SKILL 엣지 생성.
> 상세: `00_data_source_mapping.md §3.2 tech_stack`.
>
...
| `(:Vacancy)-[:NEEDS_SIGNAL]->(:SituationalSignal)` | 포지션이 필요로 하는 상황 경험 | inferred: BOOLEAN. **[v26] 생성 규칙**: `hiring_context`에서 `VACANCY_SIGNAL_ALIGNMENT` 테이블(03_mapping_features.md §2 F2)의 strong/moderate 시그널을 자동 생성. `inferred=true`는 규칙 기반 자동 생성, `inferred=false`는 JD 텍스트에서 LLM 추출 |
