  ...
  "chapters": [
    {
      ...
      "outcomes": [{"outcome_type": "SCALE", "description": "...", "confidence": 0.8}],
      "situational_signals": [{"label": "TEAM_SCALING", "confidence": 0.75}]
    }
  ]
}
```
...
- `chapters[]` **시간 순서(period_start 오름차순)** 정렬 — `chapters[0]`이 가장 오래된 경력, `chapters[-1]`이 최신 경력
...
```json
{
  ...
  "operating_model": {"speed": 0.8, "autonomy": 0.9, "process": 0.7}
}
```

> **[v8]** `required_role`, `required_skills`, `needed_signals`는 정본(01.ontology) Graph Schema에서는 관계(REQUIRES_ROLE, REQUIRES_SKILL, NEEDS_SIGNAL)로 표현되나, Data Contract에서는 GraphRAG 적재 편의를 위해 flat 필드로 제공한다. 적재 시 노드+관계로 분해하여 저장한다.

> **[v8]** `stage_label` 유효값은 정본(01.ontology)의 stage_label taxonomy: `EARLY` / `GROWTH` / `SCALE` / `MATURE` / `UNKNOWN`. "ENTERPRISE"는 유효값이 아님 (v5 리뷰 C3).

> **[v8]** `hiring_context_scope` 필드 삭제. 정본 필드명은 `seniority`이며(01.ontology/v25/01_company_context.md vacancy 스키마), `vacancy_seniority`는 사용하지 않는다.

> **[v7]** operating_model 값은 float(0.0~1.0)이다. `02.knowledge_graph/v18/03_prompt_design.md`의 Pydantic 스키마와 타입 일치. 이산형 변환(HIGH/MID/LOW)은 서빙 레이어에서 수행.

> **[v7] 유효 signal 열거값** (14개 고정 taxonomy, `01.ontology/v25/02_candidate_context.md` §2.3 정의):
