> 작성일: 2026-03-12
> 01.ontology/results/schema/v25/04_graph_schema.md §4-§9에서 이동.
...
### GQ1: vacancy_fit - 포지션이 필요로 하는 상황을 경험한 후보 탐색
...
### GQ2: stage_match - 동일 성장 단계 기업 경험 후보 탐색
...
### GQ3: 유사 경험 후보 탐색 (Vector + Graph 하이브리드)
...
### GQ4: NEEDS_SIGNAL 자동 추론 (Vacancy 생성 시)
...
| v1 채택 | **[v4] `text-embedding-005`** (Vertex AI, 768d) - GCP 네이티브, 다국어 지원. `02.knowledge_graph/v18/02_model_and_infrastructure.md`와 통일. 대안: Cohere embed-multilingual-v3.0 (1024d) |
...
> - `02.knowledge_graph/v18/01_extraction_pipeline.md`의 규모 추정(600K Person 기반)은 **v1 초기 적재 범위**(EXPERIENCED + HIGH/PREMIUM 품질) 기준이다.
...
> **[v9] v1 초기 적재 (600K) 규모 추정**: Person ~600K, Chapter ~3.4M, Organization ~100K, Outcome ~1M, 총 노드 ~5.1M, 총 관계 ~25M. Professional 8GB 충분.
...
| Chapter | ~18M | 전체 career 18.7M건. 서비스 풀(3.2M)은 EXPERIENCED 필터 후 경력 보유율 ~100%, 이력서당 평균 ~5.6건 |
...
> **사전 PoC**: 이 규모에서 GQ1~GQ5 쿼리의 응답 시간이 1초 이내인지, AuraDB Professional/Enterprise tier에서 감당 가능한지 사전 검증 필요. 파일럿에서 ~10K Person + ~50K Chapter 규모로 먼저 테스트한 후 full-scale 적재를 진행한다.
>
...
GQ1~GQ5 및 MVP Q1~Q5 쿼리 성능을 보장하기 위한 인덱스 정의.
...
```
-- GQ2 stage_match: Organization.stage_label 기반 탐색
CREATE INDEX org_stage_label FOR (o:Organization) ON (o.stage_label);

-- GQ1/GQ5 + MVP Q1/Q5: 복합 탐색
