...
```
매칭 알고리즘 설계 (2일):
  F1 stage_match (25%): Organization.stage 비교 (v25 STAGE_SIMILARITY)
...
```
-- PubSub로 S&F JD JSON 수신 -> Vacancy 적재 (v25)
UNWIND $batch AS v
...
-- REQUIRES_ROLE, REQUIRES_SKILL, NEEDS_SIGNAL (v25)
```
...
> **[v3 신규] Organization ER 3차 패스**: LinkedIn company_id를 활용한 회사명 교차 매핑 추가. resume-hub 4,479,983개 + LinkedIn 933,923개 고유 회사명을 통합하여 Organization 정규화 정확도 향상.
> 정규화 전략: BRN 1차(62%) → 회사명 argMax 2차(38%) → LinkedIn company_id 3차 교차


## W19-20: 5-피처 스코어링 + MAPPED_TO

> **[v5] 구현 참조**: 아래 `compute_match_score()`는 `01.ontology/v25/03_mapping_features.md`의 F1~F5 정의를 **v1 MVP 간소화 구현**한 것이다. 정본은 01.ontology의 정의이며, 차이점은 다음과 같다:
...
> - **[v8] F2 추가 주의사항**: 간소화 구현의 scope_match는 candidate.scope_type(IC/LEAD/HEAD/FOUNDER)과 vacancy.seniority(JUNIOR~HEAD)를 비교하는데, 이는 서로 다른 enum 공간이다. 정본 F2는 hiring_context(BUILD_NEW 등)와 situational_signals를 VACANCY_SIGNAL_ALIGNMENT 테이블 기반으로 매칭한다. v1 MVP에서는 Jaccard signal_overlap이 F2의 실질적 변별력을 제공한다.
...
def compute_match_score(candidate: dict, vacancy: dict, weights: MatchWeights) -> float:
    # F1: stage_match (v25 STAGE_SIMILARITY)
    ...
    # [v8] v1 MVP 한계: scope_type(IC/LEAD/HEAD/FOUNDER)과 seniority(JUNIOR~HEAD)는 다른 enum 공간.
    # LEAD/HEAD에서만 일치 가능. signal_overlap(아래)이 F2의 실질적 변별력을 제공한다.
    scope_match = 1.0 if candidate.get("scope_type") == vacancy.get("seniority") else 0.5
    ...
    # F4: culture_fit (INACTIVE → 가중치 재분배)
    # [v9] 정본(01.ontology/v25)의 INACTIVE 처리 정책 적용: 가중치 0으로 설정, 나머지 피처에 비례 재분배.
    # Phase 2에서 work_style_signals 수집 후 ACTIVE 전환 예정.
    culture_score = None  # INACTIVE
    ...
    # [v9] INACTIVE 피처 가중치 재분배: culture_fit이 None이면 가중치 0, 나머지 비례 확대
    active_scores = {
        "stage_match": (stage_score, weights.stage_match),
        "vacancy_fit": (vacancy_score, weights.vacancy_fit),
        "domain_fit": (domain_score, weights.domain_fit),
        "culture_fit": (culture_score, weights.culture_fit),
        "role_fit": (role_score, weights.role_fit),
    }
    active = {k: v for k, v in active_scores.items() if v[0] is not None}
    total_active_weight = sum(w for _, w in active.values())
    overall = sum(score * (weight / total_active_weight) for score, weight in active.values()) if total_active_weight > 0 else 0
    return round(min(max(overall, 0), 1), 4)

def compute_ranking_score(candidate: dict, vacancy: dict, weights: MatchWeights) -> dict:
    """overall_match_score에 freshness_weight를 적용하여 ranking_score 산출."""
    overall = compute_match_score(candidate, vacancy, weights)
    # [v8] freshness_weight: 01.ontology/v25/00_data_source_mapping.md §3.5
    # Person 노드의 freshness_weight (0.3~1.0) 적용
    freshness = candidate.get("freshness_weight", 1.0)
    ranking_score = round(overall * freshness, 4)
    return {
