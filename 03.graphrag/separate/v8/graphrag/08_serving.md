> 작성일: 2026-03-12
> 01.ontology/results/schema/v25/03_mapping_features.md §4.2, §5에서 이동.
...
CREATE TABLE context.mapping_features (
  ...
  -- [v7] 랭킹 (§1 freshness_weight 적용)
  ranking_score FLOAT64,           -- overall_match_score × freshness_weight
  freshness_weight FLOAT64,        -- Person.freshness_weight (0.3~1.0)
  low_confidence_flag BOOLEAN,     -- 활성 피처 ≤ 2개 시 true (01.ontology/v25/03_mapping_features.md §4 판정 규칙)
...
-- [v7] ranking_score = overall_match_score × freshness_weight (§1 정의)
-- overall_match_score로 순수 적합도 비교, ranking_score로 신선도 반영 랭킹
ORDER BY mf.ranking_score DESC
LIMIT 50;
