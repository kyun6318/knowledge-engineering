...
> **[v7]** `org_stage` 유효값은 정본(01.ontology)의 stage_label taxonomy: `EARLY` / `GROWTH` / `SCALE` / `MATURE` / `UNKNOWN`. "ENTERPRISE"는 유효값이 아님 (v5 리뷰 C3).

> **[v7]** `hiring_context_scope` → `vacancy_seniority`로 필드명 변경. 정본(01.ontology/v24/01_company_context.md)의 vacancy.seniority 필드와 통일. 유효값: JUNIOR/MID/SENIOR/LEAD/HEAD/UNKNOWN.

> **[v7]** operating_model 값은 float(0.0~1.0)이다. `02.knowledge_graph/v17/03_prompt_design.md`의 Pydantic 스키마와 타입 일치. 이산형 변환(HIGH/MID/LOW)은 서빙 레이어에서 수행.

> **[v7] 유효 signal 열거값** (14개 고정 taxonomy, `01.ontology/v24/02_candidate_context.md` §2.3 정의):
