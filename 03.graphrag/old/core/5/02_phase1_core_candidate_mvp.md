...
>
> **v4 대비 변경**:
> - ★ v5 U2: mask_phones() 토큰 형식 변경 시 다중 패턴 간섭 확인 필요 주석 추가
>
> **v3 대비 변경 (v4에서 반영 완료)**:
...
def mask_phones(text: str) -> tuple[str, dict]:
    ...
    ★ v5 U2: PHONE_PATTERNS를 순차 적용하므로, 토큰 형식([PHONE_xxx])이
    후속 패턴에 매칭되지 않아야 함. 현재 [PHONE_000] 형식은 안전함
    (PHONE_PATTERNS[1]이 0[2-6]으로 시작하므로 매칭 불가).
    토큰 형식을 변경할 경우 다중 패턴 간 간섭 여부를 반드시 확인할 것.
    """
    mapping = {}
    counter = [0]
