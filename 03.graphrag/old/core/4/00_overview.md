# GraphRAG Core 확장 계획 v4 — v3 리뷰 반영 + 실현 가능성 보강

> **목적**: Core Candidate-Only MVP(6주)를 출발점으로, standard의 모든 기능을
> **데이터 레이어 확장 순서**로 추가하여 에이전트 조회 역량을 점진적으로 확대.
>
> **v3 대비 주요 변경** (core.3 리뷰 R1~R8 반영):
> - R1: PII 마스킹 오프셋 버그 수정 (re.sub 콜백 방식)
> - R2: 비용 문서 단일 소스 통일 (06_cost.md 기준)
> - R3: CMEK 버킷 생성을 Phase 0 → Phase 1-B로 이동
> - R4: Hybrid 섹션 분리 LLM 폴백도 Batch API로 묶어 처리
> - R5: Phase 2를 9주로 1주 연장 (Phase 3을 6주로 축소, 총 27주 유지)
> - R6: 600K 처리 우선순위 전략 명시 (DB 500K 먼저 → 파일 100K 후순위)
> - R7: 소프트 삭제 도입 시 Cypher 쿼리 마이그레이션 태스크 추가
> - R8: DETACH DELETE 4단계 → 2단계 통합 (트랜잭션 원자성 확보)
> - O3: 가중치 튜닝 Grid search → 수동 비교 우선, 자동화 참고
> - O7: Phase 0 산출물 Must/Should 분류
>
> 작성일: 2026-03-11
...
| 문서 | 내용 |
|------|------|
| `00_overview.md` (본 문서) | 전체 구조, v3→v4 변경점, 데이터 확장 로드맵 |
...
| # | 변경 | v3 | v4 | 영향 |
|---|------|----|----|------|
| R1 | PII 마스킹 오프셋 버그 수정 | `re.finditer` 순차 치환 (오프셋 오류) | **`re.sub` 콜백 방식** | PII 누출 방지 (HIGH) |
| R2 | 비용 문서 단일 소스 | Overview §14와 06_cost 간 $70~90 불일치 | **06_cost.md를 Single Source of Truth** | 관리 단순화 |
| R3 | CMEK 버킷 생성 시점 | Phase 0 (PoC 전) | **Phase 1-B** (Go 판정 후) | DE 0.5일 여유 확보 |
| R4 | LLM 폴백 Batch API | 개별 호출만 설계 | **Batch API로 묶어 처리** | 30K건 처리 시간 15h→수시간 |
| R5 | Phase 2 기간 | 8주 (여유 0일) | **9주** (Phase 3 6주로 축소) | 기본 시나리오 여유 +1주 |
| R6 | 처리 우선순위 | 미명시 | **DB 500K 먼저 → 파일 100K 후순위** | 적응형 호출 비율 조기 검증 |
| R7 | 소프트 삭제 쿼리 마이그레이션 | "모든 쿼리에 필터 필수" 서술만 | **Phase 4-4에 마이그레이션 태스크 명시** | 누락 방지 (+0.5일) |
| R8 | DETACH DELETE 단계 | 4단계 (트랜잭션 4회) | **2단계** (원자성 확보) | 트랜잭션 실패 위험 ↓ |

### 3.2 v3 리뷰 추가 반영 (O3, O7)

| # | 변경 | v3 | v4 | 영향 |
|---|------|----|----|------|
| O3 | 가중치 튜닝 | Grid search 자동화 코드 | **수동 비교 우선, 자동화 참고** 주석 | 50건 규모에서 통계적 적합 |
| O7 | Phase 0 산출물 | 20개 항목 미분류 | **Must/Should 분류** | 1주 안 필수 항목 집중 |
...
> ★ v4: 비용 상세는 `06_cost_and_monitoring.md` §1 참조. Overview에서는 총합만 기재.

| 항목 | v3 | v4 | 차이 |
|------|-----|-----|------|
| Phase 2 인프라 (9주, +1주) | $319~599 | **$359~669** | +$40~70 (1주 추가) |
| 기타 | 동일 | 동일 | — |
| **총합계** | $5,523~9,138 | **$5,563~9,208** | +$40~70 |
...
> ★ v4: 비용 상세는 `06_cost_and_monitoring.md` 참조. 이 섹션은 **참조 링크만 유지**.
>
> **비용 Single Source of Truth: `06_cost_and_monitoring.md` §1** (R2)
>
> 총합계 범위: **~$5,563~9,208** (v3: $5,523~9,138 대비 +$40~70, Phase 2 1주 연장분)
>
> 원화: **~762~1,262만**
