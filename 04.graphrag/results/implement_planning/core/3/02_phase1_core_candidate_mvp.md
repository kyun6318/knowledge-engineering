...
>
> **v2 대비 변경**:
> - N1: AuraDB Free Auto-Pause 대응 — Cloud Scheduler health check 12h 주기 설정
> - N2: API 응답 PII 필드 정의서 추가 — /candidates/{id} PII 필터링
> - v12 M1: 적응형 LLM 호출 적용 (1-pass / N+1 pass, PoC 검증 기반)
> - v12 M2: v19 canonical 관계명 전면 적용 (PERFORMED_ROLE, OCCURRED_AT 등)
> - v12 S2: PII 매핑 테이블 GCS CMEK 저장
> - v12 S4: 전화번호 8종 정규식 적용
> - v12 S5: INACTIVE 필드 프롬프트 제외 (structural_tensions, work_style_signals)
>
...
```python
# src/pii/masker.py — v3: v12 S2 + S4 적용
...
> ★ v12 프롬프트 + 적응형 호출 전략 적용.

### 적응형 LLM 호출 (v12 M1)

```python
# src/extractors/candidate_extractor.py — v3: v12 M1
...
### Graph 적재 코드 (★ v3: v19 관계명)

```python
# src/graph/load_candidate.py — v3: v19 canonical 관계명
...
```bash
# ★ v3 N1: Cloud Scheduler로 12시간마다 health check
# Phase 1 API 배포 후 설정
...
# 효과: Neo4j 72시간 비활성 방지 → API 중단 방지
```

### ★ 에이전트 서빙 API + PII 필터링 (N2)

```python
# src/api/main.py — v3: PII 필터링 미들웨어 추가

from fastapi import FastAPI, Request
from fastapi.middleware.cors import CORSMiddleware

app = FastAPI(title="GraphRAG API", version="1.1")
...
```cypher
-- Q1: 스킬 기반 검색 (★ USED_SKILL 유지, 변경 없음)
...
-- Q3: 회사 기반 검색 (★ AT_COMPANY → OCCURRED_AT)
...
Week 6 중반 체크포인트:
  ...
  □ ★ PII 매핑 GCS CMEK 저장 동작 확인 (v12 S2)
