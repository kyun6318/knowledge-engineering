# Go/No-Go + 의사결정 포인트 (S&F ↔ GraphRAG 공동)
...
```
매주 월요일:
  1. S&F 처리 현황 (진행률, 완료건수, 오류건수)
  2. GraphRAG 적재 현황 (노드/엣지 수, 쿼리 성능)
  3. Data Contract 이슈 (스키마 변경 요청, 필드 누락)
  4. 다음 주 계획 + 블로커 식별
```
